...
* 將 FlexVol Volume 轉換為 FlexGroup Volume 是一個非中斷操作。轉換通常只需幾秒鐘，無需將 Volume 離線或重新掛載共用和匯出，因此用戶端可以繼續存取資料。
...
** 在 ONTAP 9.10.1 之前的版本中，待轉換的 FlexVol Volume 所在的 SVM 目前使用 SVM-DR。從 ONTAP 9.10.1 開始，您可以link:convert-flexvol-svm-dr-relationship-task.html["將 FlexVol Volume 轉換為 SVM-DR 來源上的 FlexGroup Volume"]。
** 存在 NetApp FlexClone® Volume，且該 FlexVol Volume 為父 Volume。正在進行轉換的 Volume 不能是父 Volume 或 Clone。
...
** 該 Volume 上的自主勒索軟體保護功能已停用。在轉換完成之前，請勿再次啟用該功能。
...
[NOTE]
====
轉換非常大的 FlexGroup 磁碟區會導致 FlexGroup 磁碟區成員元件非常滿，這可能會造成效能問題。如需更多資訊，請參閱 TR link:https://www.netapp.com/media/12385-tr4571.pdf["《英文版-最佳實務做法與實作指南》FlexGroup"^] 中名為「何時不應建立 FlexGroup 磁碟區」的部分。
