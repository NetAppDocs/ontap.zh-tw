...
.. 定義鏡射和資料保險箱原則的規則、以決定在初始化和更新作業期間傳輸哪些 Snapshot 複本： `snapmirror policy add-rule -vserver svm_name -policy policy_for_rule - snapmirror-label snapmirror-label -keep retention_count -schedule schedule`
