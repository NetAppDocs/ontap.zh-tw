...
對於下列平台，預設情況下，僅對升級至 ONTAP 9.15.1 或更高版本之前已啟用儲存效率的現有精簡配置磁碟區啟用儲存效率。您可以使用 ONTAP CLI 或 ONTAP REST API 方法在新建立的磁碟區上啟用儲存效率。使用磁碟區移動或 SnapMirror 技術移轉的資料，僅當原始平台啟用了壓縮時，才會轉換為 32k 內嵌壓縮。
