...
[role="lead"]
對於每個主要的 ONTAP 版本，常用的 CLI 指令（ONTAP 手冊頁或手冊頁）都會捆綁到link:https://docs.netapp.com/us-en/ontap-cli/["《指令參考》ONTAP"^] 。這些命令參考資料說明如何在每ONTAP 個版本的各個版本中使用CLI命令。手冊頁也可以在 ONTAP 命令列中使用 `man`命令。如link:https://docs.netapp.com/us-en/ontap-cli/man.html["指令參考資料ONTAP"^]需詳細 `man`資訊，請參閱。
