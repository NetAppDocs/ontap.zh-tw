...
summary: 您的 ONTAP 儲存容量可以實體空間或邏輯空間來衡量。這些測量結果的報告方式取決於您的 ONTAP 版本以及您使用的是 System Manager 還是命令列介面（CLI）。了解與容量相關的各種術語、實體容量和邏輯容量之間的差異以及容量類型的報告方式，對於監控儲存使用情況至關重要。 
---
= 瞭解 ONTAP 容量報告和測量
:hardbreaks:
:toclevels: 1
...
[role="lead"]
您的 ONTAP 儲存容量可以以實體空間和邏輯空間來衡量。這些測量值的報告方式取決於您的 ONTAP 版本以及您使用的是 System Manager 還是命令列介面（CLI）。了解與容量相關的各種術語、實體容量和邏輯容量之間的差異以及容量類型的報告方式，對於監控儲存使用情況至關重要。
...
根據您所使用的 ONTAP 版本，ONTAP CLI 中回報的實體已用空間會有所不同。在 ONTAP 9.13.1 及更早版本中， `physical used` 參數顯示於  `volume show` 指令輸出中，包含套用於該 volume 的所有儲存效率節省。自 ONTAP 9.14.1 起， `physical used` 參數不再包含如溫度感知儲存（TSSE）、即時壓縮、壓縮整合及跨 volume 重複資料刪除等集合層級的儲存效率節省，因此無法準確反映該 volume 的實體已用空間。在 ONTAP 9.14.1 及更新版本中，您應該使用  `vol show-footprint` 指令搭配  `effective_total_footprint` 參數，以檢視 volume 的正確實體已用空間。
...
.3+| 9.14.1 及更高版本 | `volume show-footprint` | `effective_total_footprint` | 反映實際使用的磁碟空間,包括從 aggregate 節省的空間。 


| `volume show-space` | `physical used` | 反映磁碟區層級的儲存效率節省，包括來自 aggregate 的節省。 


| `volume show` | `physical used` | 不包括彙總層級的儲存效率節省，因此無法準確反映 link:../volumes/enable-temperature-sensitive-efficiency-concept.html["啟用 TSSE 的平台"] 的磁碟區實體使用空間。 


.3+| 9.13.1 及更早版本 | `volume show-footprint` | `effective_total_footprint` | 反映實際使用的磁碟空間,包括從 aggregate 節省的空間。 


| `volume show-space` | `physical used` | 僅反映磁碟區層級儲存效率的節省。 


| `volume show` | `total physical used` | 僅反映磁碟區層級儲存效率的節省。 
|===
.相關資訊
* 深入瞭解 link:https://kb.netapp.com/on-prem/ontap/OHW/OHW-KBs/What_is_the_Change_in_Physical_Used_Space_Reporting_for_9.14.1_and_later["9.14.1 及更高版本實體使用空間報告的變更"^]。
* 深入瞭解 link:../volumes/commands-display-space-usage-reference.html["用於顯示空間使用資訊的命令"]。




== System Manager 中的容量報告

根據 ONTAP 版本的不同，System Manager 中顯示的已用容量測量值也會有所不同。從 ONTAP 9.7 開始，System Manager 同時提供物理容量和邏輯容量的測量數據。
