...
ONTAP 實作可在 NetApp 設計的 FAS 、 AFF 或 C 系列應用裝置、商業硬體（ ONTAP Select ）、私有雲、公有雲或混合雲（ NetApp 私有儲存或 Cloud Volumes ONTAP ）上執行。專業實作提供同級最佳的融合式基礎架構FlexPod （「Datacenter」）、並可存取第三方儲存陣列FlexArray （「整合式虛擬化」）。
