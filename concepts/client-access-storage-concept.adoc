...
ONTAP 支援安全性斷言標記語言（SAML）進行遠端使用者驗證。它支援多種常用的身份提供程序（IdPs）。有關支援的 IdPs 以及啟用 SAML 身份驗證的說明的更多資訊，請參閱 link:../system-admin/configure-saml-authentication-task.html["設定SAML驗證"]。
