...
[role="lead"]
_SnapMirror cloud_ 是一種備份和復原技術，專為希望將其資料保護工作流程遷移到雲端或本地物件儲存解決方案的 ONTAP 使用者而設計。正在擺脫傳統磁帶備份架構的組織可以使用物件儲存作為長期資料保留和歸檔的替代儲存庫。SnapMirror cloud 提供 ONTAP 到物件儲存的複製功能，作為永久增量備份策略的一部分。
