...
未在一或多個 NAS 儲存 VM 上設定原生 FPolicy 。
 a| 
* 推薦 * ： * 重要 * ：封鎖副檔名可能會導致非預期的結果。從 9.11.1 開始、您可以為儲存 VM 啟用原生 FPolicy 、以封鎖已知用於勒索軟體攻擊的超過 3000 個副檔名。  https://docs.netapp.com/us-en/ontap/insights-configure-native-fpolicy-task.html["設定原生 FPolicy"] 在 NAS 儲存 VM 中、控制允許或不允許寫入環境磁碟區的副檔名。
