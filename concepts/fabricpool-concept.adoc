...
summary: 許多 NetApp 客戶儲存了大量很少存取的資料。我們稱之為冷資料。客戶還有經常存取的資料，我們稱之為熱資料。理想情況下，為了獲得最佳效能，您應該將熱資料儲存在速度最快的儲存設備上。冷資料可以移至速度較慢的儲存設備，只要在需要時可以立即使用即可。了解如何區分哪些資料是熱資料、哪些是冷資料。 
---
= FabricPool
:allow-uri-read: 
:icons: font
...
.相關資訊
link:../concept_cloud_overview.html["系統管理程式文件FabricPool"]

https://docs.netapp.com/us-en/data-services-cloud-tiering/index.html["NetApp雲端分層"^]
