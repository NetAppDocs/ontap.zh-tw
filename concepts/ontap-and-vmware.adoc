...
NOTE: 從 ONTAP 9.15.1 開始，您可以使用 SCSI-3 為虛擬 Volume 建立持續保留。此功能僅在使用 ONTAP tools for VMware vSphere 9 搭配 Windows Server 容錯移轉叢集（WSFC）時才支援。
...
VMware Live Site Recovery（原名 Site Recovery Manager，簡稱 SRM）提供災難復原功能。Live Site Recovery 與 ONTAP tools for VMware vSphere 整合，以便存取和利用 ONTAP 資料管理功能。
