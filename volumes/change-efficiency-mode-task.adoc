...
[role="lead"]
從ONTAP 9.10.1 開始，磁碟區級儲存效率模式 _default_ 和 _efficient_ 僅對AFF系統支援。這些模式提供了兩種選擇：檔案壓縮（預設），這是建立新AFF磁碟區時的預設模式；或溫度敏感儲存效率（高效），它啟用溫度敏感儲存效率 (TSSE)。


NOTE: TSSE 僅在精簡配置磁碟區上支援。link:enable-temperature-sensitive-efficiency-concept.html["了解更多關於TSSE的信息"]。
