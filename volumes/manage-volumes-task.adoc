...
.步驟
. 在 System Manager 中，選取 *Storage > Volumes* 。
+
隨即顯示Volume清單。

+
SVM 管理員看到的選項有限，且可能沒有新增磁碟區的權限。
...
新增Volume
 a| 
選擇 image:icon_add_blue_bg.gif["新增圖示"]。請參閱 link:../task_admin_add_a_volume.html["新增Volume"]。
...
* 選取 image:icon_delete_with_can_white_bg.gif["刪除圖示"] 以刪除所選磁碟區。
* 選擇 image:icon_protect.gif["保護圖示"] 可為選定的磁碟區指派保護原則。
* 選取 image:icon-more-kebab-white-bg.gif["更多圖示"] 以選擇要對所有選定磁碟區執行的下列其中一項動作：
+
** 啟用配額
** 離線
** 移動
...
管理單一Volume
 a| 
在磁碟區旁邊，選擇 image:icon_kabob.gif["功能表選項圖示"]，然後選擇下列其中一項動作來執行：
...
選擇磁碟區名稱旁邊的 image:icon-edit-pencil-blue-outline.png["編輯圖示"]，然後修改磁碟區名稱。
