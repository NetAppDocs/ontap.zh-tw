...
---
= 在四節點或八節點 MetroCluster 組態上手動執行 ONTAP NDU
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
對四節點或八節點 MetroCluster 組態進行手冊不中斷營運的升級 （NDU），包括準備更新、同時更新一個或兩個 DR 群組中的每個 DR 配對，以及執行升級後工作。

.關於這項工作
* 您必須使用 ONTAP 命令列介面 （CLI） 來執行此工作。
* 此工作適用於下列組態：
+
...
----
+
以下範例顯示系統映像 2（目標 ONTAP 映像）是 node_A_1 上的預設和目前版本：
...
----
+
以下範例顯示系統映像 2（目標 ONTAP 映像）是 node_B_1 上的預設和目前版本：
...
----
+
以下範例顯示系統映像 2 （目標 ONTAP 映像）是 node_A_2 上的預設和目前版本：
...
----
+
以下範例顯示系統映像 2（目標 ONTAP 映像）是 node_B_2 上的預設和目前版本：
