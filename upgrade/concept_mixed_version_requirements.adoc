...
本內容適用於支援 NetApp 平台系統的 ONTAP 版本、例如 AFF A 系列和 C 系列、 ASA 和 FAS 、以及 C 系列系統。此處的資訊不適用於 ONTAP 雲端版本（ 9.x ）、例如 9.12.0 。
...
如果您打算將新節點新增至叢集、而這些節點需要比叢集目前執行版本更高的最低 ONTAP 版本、則在新增節點之前、您需要在叢集中現有節點上執行任何可能的軟體升級。
