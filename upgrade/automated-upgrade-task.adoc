...
[role="lead"]
當您執行自動不中斷升級 （ANDU） 時，ONTAP 會自動在每個節點上安裝目標 ONTAP "映像"，驗證叢集是否可以成功升級，然後根據叢集中的節點數在背景執行 xref:concept_upgrade_methods.html[批次或循環升級]。
...
IMPORTANT: 在 ANDU 啟動之前修改 `storage failover modify-auto-giveback`命令選項的設定不會影響升級過程。ANDU 程序會在更新所需的接管/歸還期間忽略此選項的任何預設值。例如，在啟動 ANDU 之前將 `-autogiveback`設為 false 不會中斷歸還前的自動升級。深入瞭解 `storage failover modify-auto-giveback`，請參閱link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=storage+failover+modify-auto-giveback["指令參考資料ONTAP"^]。
...
* （可選）link:../update/pre-stage-system-firmware.html["預先配置系統韌體，以便進行單一重新開機升級"]
+
從 ONTAP 9.17.1 開始，如果在執行 ANDU 之前節點的開機設備上已存在較新的 BIOS 或 LOADER 韌體，升級程序將保留較新的韌體，而不是以 ONTAP 映像中隨附的版本覆寫。如此一來，ONTAP 軟體升級和韌體更新即可在單一重新開機週期內完成。
...
+
對於匿名 FTP，您必須以 \ftp://anonymous@ftpserver 格式指定 URL。
...
.. 完成驗證所識別的所有必要行動。
+
系統會顯示完整的自動升級預先檢查清單，以及在開始升級程序之前應解決的任何錯誤或警告。您必須先解決所有錯誤，才能繼續進行升級。最佳實務做法是同時解決警告。
...
====
.接下來呢？
如果您已預先暫存韌體，請使用 `system node firmware bootmedia show`命令驗證每個節點上的預期韌體版本是否為作用中。
