...
若要執行的自動升級預先檢查link:../upgrade/concept_upgrade_paths.html#types-of-upgrade-paths["直接多跳升級"]、您只需下載目標 ONTAP 版本的軟體套件。在您開始實際升級之前、您不需要載入中繼 ONTAP 版本。例如、如果您要執行自動升級前檢查、以從 9.8 升級至 9.13.1 、則需要下載 ONTAP 9.13.1 的軟體套件。您不需要下載 ONTAP 9.12.1 的軟體套件。
...
NOTE: 如果您正在執行link:../upgrade/concept_upgrade_paths.html#types-of-upgrade-paths["直接多跳升級"]、請使用目標 ONTAP 套件進行驗證。您不需要個別驗證中間升級映像。例如、如果您要從 9.8 升級至 9.13.1 、則應使用 9.13.1 套件進行驗證。您不需要個別驗證 9.12.1 套件。
