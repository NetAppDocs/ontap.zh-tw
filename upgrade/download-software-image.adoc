...
|===
.關於這項工作
* 如果您使用執行自動不中斷升級 link:concept_upgrade_paths.html#types-of-upgrade-paths["直接多跳升級途徑"]，您需要下載中級 ONTAP 版本和升級所需的目標 ONTAP 版本的軟體套件。  例如、如果您要從 ONTAP 9.8 升級至 ONTAP 9.13.1 、則必須同時下載 ONTAP 9.12.1 和 ONTAP 9.13.1 的軟體套件。  請參閱 link:concept_upgrade_paths.html#supported-upgrade-paths-for-on-premises-ontap-and-ontap-select["支援的升級路徑"] 若要判斷您的升級路徑是否需要下載中介軟體套件。
