...
----
+
** 如果您執行的是 link:https://docs.netapp.com/us-en/ontap/upgrade/concept_upgrade_paths.html#types-of-upgrade-paths["直接多跳升級"] 使用 package_version_number 的目標 ONTAP 版本。例如、如果您要從 ONTAP 9.8 升級至 9.13.1 、請使用 9.13.1 做為 package_version_number 。
...
----
+
如果您要升級4節點或8節點MetroCluster 的BIOS組態、「cluster image show-update-progress」命令只會顯示執行命令之節點的進度。您必須在每個節點上執行命令、才能查看個別節點的進度。
