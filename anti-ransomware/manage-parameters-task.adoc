...
NOTE: 若要減少誤報數量，請前往 *Storage > Volumes > Security > Configure workload characteristics*，然後停用 *Monitor new file types*。此設定在 ONTAP 9.14.1 P7、9.15.1 P1、9.16.1 及更高版本中預設為停用。從 ONTAP 9.18.1 P2 開始，System Manager 不再顯示 ARP 工作負載特性，包括基準和激增統計資料。激增統計資料仍可使用 ONTAP CLI 和 REST API 選項進行進階分析。
