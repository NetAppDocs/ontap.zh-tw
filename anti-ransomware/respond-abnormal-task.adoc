...
在這兩種情況下、正常監控會在更新及清除通知後繼續進行；您的評估會由Arp記錄、使用新的檔案類型更新記錄、並用於未來的分析。不過、若發生可疑的攻擊、您必須先判斷是否為攻擊、如果是攻擊、請加以回應、然後在清除通知之前還原受保護的資料。 link:index.html#how-to-recover-data-in-ontap-after-a-ransomware-attack["深入瞭解如何從勒索軟體攻擊中恢復"]。
