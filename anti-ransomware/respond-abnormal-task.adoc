...
.. 執行下列其中一項：
+
*** 對於檔案類型警告，請選取 *Update and Clear Suspected File Types* 。
*** 對於熵尖峰，選擇*標記為假陽性*。
+
...
NOTE: 從ONTAP 9.13.1 開始，如果您使用 MAV 來保護 ARP 設置，清除可疑項目操作會提示您獲得一個或多個其他管理員的批准。link:../multi-admin-verify/request-operation-task.html["必須收到所有管理員的核准"]與 MAV 審批小組相關，否則操作將會失敗。
...
. 從 9.18.1 版本開始，您可以確定以下狀態： `clear-suspect`手術：
+
[source, cli]
----
