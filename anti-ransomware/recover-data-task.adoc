...
.有關 SnapMirror 關係和 ARP 快照的更多資訊
[%collapsible]
====
如果受 ARP 保護的磁碟區屬於SnapMirror關係，則從快照還原磁碟區後，您需要手動更新該磁碟區的所有映像副本。如果跳過此步驟，鏡像副本可能會變得不可用，需要刪除並重新建立。

如果受 ARP 保護的磁碟區是 ONTAP 9.19.1 RC 中 SnapMirror 同步或 SnapMirror 主動同步關係的一部分,則需要考慮其他還原因素:

* ARP 快照僅在主磁碟區上建立和保留。它們不會作為 SnapMirror 同步或 SnapMirror 主動同步的一部分複製到輔助磁碟區。
* 故障轉移期間，ARP 啟用狀態不會複製。您需要在故障轉移後的復原磁碟區上 link:enable-task.html["再次啟用 ARP"]。
* 磁碟區層級還原 (`volume snapshot restore`) 在 SnapMirror 同步或 SnapMirror 主動式同步磁碟區上可能需要您暫時靜止或中斷 SnapMirror 同步或 SnapMirror 主動式同步關係。
* 在許多情況下，您可以透過使用 FlexClone 或從 ARP 或其他快照於主要磁碟區執行檔案層級還原作業，來避免中斷 SnapMirror synchronous 或 SnapMirror active sync。
...
+
如果磁碟區是 ONTAP 9.19.1 RC 中 SnapMirror 同步或 SnapMirror 主動式同步 SAN 關係的一部分，並且您打算執行磁碟區層級還原，請在開始還原之前依照 SnapMirror 同步或 SnapMirror 主動式同步文件使其靜止或link:../data-protection/make-destination-volume-writeable-task.html["中斷保護關係"]，然後在還原完成後重新建立保護。
...
NOTE: 如果您使用 `volume snapshot restore`下述命令從早期快照還原資料，則只需在還原之前釋放反勒索軟體 SnapLock 即可。如果您使用 FlexClone、單一檔案 SnapRestore 或其他方法還原資料，則無需執行此操作。
...
. 如果磁碟區是 ONTAP 9.19.1 RC 中 SnapMirror 同步或 SnapMirror 主動式同步 SAN 關係的一部分，且您打算使用 `volume snapshot restore`執行磁碟區層級還原，請在執行還原作業之前，根據 SnapMirror 同步或 SnapMirror 主動式同步文件進行靜止或link:../data-protection/make-destination-volume-writeable-task.html["中斷 SnapMirror 同步或 SnapMirror 主動同步關係"]。
. 列出磁碟區中的快照：
+
[source, cli]
----
