...
有關傳輸協定支援、工作負載適用性和版本特定需求，請參閱 link:use-cases-restrictions-concept.html["ARP 使用案例與考量事項"]。
...
.新磁碟區上的自動預設啟用
從 ONTAP 9.18.1 開始，對於 AFF A 系列和 AFF C 系列、ASA 和 ASA r2 系統，所有新建磁碟區預設會自動啟用 ARP。此預設會自動啟用 ARP 的功能不適用於link:use-cases-restrictions-concept.html#unsupported-configurations["不支援的磁碟區或組態"]。您必須<<manual-default-enablement,手動啟用 ARP>>。有關升級時預設啟用功能如何生效的詳細資訊，請參閱link:use-cases-restrictions-concept.html#arp-upgrade-considerations["ARP 版本特定的升級考量事項"]。


NOTE: 從 ONTAP 9.19.1 開始，預設 ARP 能力開發適用於包含 SnapMirror 同步和 SnapMirror 主動同步 SAN 關係的叢集。在 ONTAP 9.18.1 中，這些組態的預設能力開發已停用。對於跨叢集 SnapMirror 同步關係，來源和目的地叢集均升級至 ONTAP 9.19.1 後，預設能力開發才會啟動。即使具備 SnapMirror 同步支援，AFX 系統上預設也不會啟用 ARP。
...
評估
 a| 
系統會進行兩到四週的評估期，以確定基線加密行為。在建立基線閾值期間，可能會出現偵測和警報。您可以透過執行  `security anti-ransomware volume show` 命令並檢查  `Block device detection status` 來確定評估期間是否已完成。

link:respond-san-entropy-eval-period.html["了解 ONTAP ARP/AI 針對區塊裝置工作負載的評估期"]。
 a| 
...
.ARP 快照版本差異
* *修改保留設定*：從 ONTAP 9.11.1 開始、您可以link:modify-automatic-snapshot-options-task.html["修改 Snapshot 的保留設定"]。
* *間隔和異常回應*：從 ONTAP 9.17.1 開始，NAS 和 SAN 磁碟區都會定期產生 ARP 快照，並且在偵測到異常時也會產生快照，這始終是標準回應機制。ONTAP 在 ARP 快照名稱前面加上一個前綴，以便於識別。
* *SnapMirror 同步和主動同步*：從 ONTAP 9.19.1 開始，為參與 SnapMirror 同步或 SnapMirror 主動同步關係的 Volume 所建立的 ARP 快照遵循下列附加規則：
+
** ARP 快照會建立並保留在主磁碟區上。
** ARP 快照不會作為 SnapMirror 同步或 SnapMirror 主動同步的一部分複製到輔助磁碟區。您需要在故障轉移後於復原磁碟區上 link:enable-task.html["再次啟用 ARP"]。





NOTE: 在 SnapMirror 同步或 SnapMirror active sync SAN 磁碟區上執行磁碟區層級還原 (`volume snapshot restore` 時，可能需要暫時靜止或中斷 SnapMirror 同步或 SnapMirror active sync 關係。在許多情況下，您可以透過使用 FlexClone 或從 ARP 或其他主磁碟區快照進行檔案層級還原作業，來避免中斷 SnapMirror 同步或 SnapMirror active sync。

link:use-cases-restrictions-concept.html#snapmirror["深入瞭解 SnapMirror 同步和 SnapMirror 主動同步的支援"]。
