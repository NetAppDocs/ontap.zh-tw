...
* 加密資料、拒絕對檔案提供服務。
除非支付贖金、否則攻擊者會保留對此資料的存取權。
* 機密專屬資料遭竊。
除非支付贖金、否則攻擊者有可能將此資料發佈至公有網域。
...
在威脅較低的情況下、 ONTAP 會偵測到異常狀況、並建立磁碟區的快照、以建立最佳的恢復點。ARP Snapshot 使用 `anti-ransomware-backup` 例如、標記和可透過其名稱識別 `Anti_ransomware_backup.2022-12-20_1248`。

ONTAP 執行分析報告、判斷異常狀況是否與勒索軟體設定檔相符、威脅就會升級至中度。系統管理員的「事件」 ** 區段會記錄並顯示低層級的威脅、但 ONTAP 不會傳送低威脅的警示。當攻擊可能性中等時、 ONTAP 會產生 EMS 通知、提示您評估威脅。如需詳細資訊、請參閱 xref:respond-abnormal-task.html[回應異常活動]。
