...
[role="lead"]
從 ONTAP 9.10.1 開始，ONTAP 管理員可以啟用自主勒索軟體防護（ARP）功能，在 NAS（NFS 和 SMB）環境中執行工作負載分析，主動偵測並警告可能顯示勒索軟體攻擊的異常活動。從 ONTAP 9.17.1 開始，ARP 也支援區塊設備磁碟區，包括包含 LUN 或 NVMe 命名空間的 SAN 磁碟區，以及包含來自 VMware 等 Hypervisor 的虛擬磁碟的 NAS 磁碟區。從 ONTAP 9.17.1P5 開始，Hyper-V、KVM 和 OpenStack Hypervisor 也受到支援，ARP 還為 NAS 磁碟區中包含的虛擬磁碟新增了評估模式（請參閱<<NAS 環境和模式類型>>）。ARP 直接內建在 ONTAP 中，確保與 ONTAP 的其他功能實現整合控制和協調。ARP 即時運作，在資料寫入或從檔案系統讀取時進行處理，並快速偵測和回應潛在的勒索軟體攻擊。
