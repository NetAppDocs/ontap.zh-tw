...
* 從 ONTAP 9.10.1 到 9.15.1，具有超過 4,000 個唯一檔案副檔名的磁碟區上的 NAS 工作負載。
+
原始的 ARP 模型每個磁碟區最多可以分析 4,000 個檔案副檔名。如果磁碟區上唯一的檔案副檔名數量超過此限制，ARP 將無法有效偵測勒索軟體活動，且此工作負載被認為不適合 ARP。從 ONTAP 9.16.1 開始，ARP/AI 不再受此限制。
...
| ARP/AI 預設啟用 ^2^ | ✓ | ✓ |  |  |  |  |  |  |  |  
|===
^1^ ONTAP 9.16.1 和 9.17.1 不支援 FlexGroup 磁碟區的 ARP/AI；從 ONTAP 9.18.1 開始，FlexGroup 磁碟區使用 ARP/AI 模型。有關升級到這些版本時的行為，請參閱<<arp-upgrade-considerations,ARP 版本特定的升級考量事項>>。

^2^ 如需升級至 ONTAP 9.18.1 後 ARP 預設啟用行為的相關資訊，請參閱 <<arp-upgrade-considerations,ARP 版本特定的升級考量事項>>。

^3^ 從 ONTAP 9.19.1 開始，ARP/AI 支援 FAS、AFF A 系列、AFF C 系列和 AFX 系統上 NAS 和 SAN 的 SnapMirror 同步關係中的主要磁碟區，以及 AFF A 系列、AFF C 系列和 ASA r2 系統上 SnapMirror 主動同步 SAN 關係中的主要磁碟區。如需詳細資訊，請參閱 <<SnapMirror>>。
...
* 在 ONTAP 9.19.1 中，ARP 設定狀態和 ARP 快照不會作為 SnapMirror 同步或 SnapMirror 主動同步的一部分複製到次要磁碟區。
...
在 ONTAP 9.10.1 和 9.11.1 版本中，SnapMirror 不會將 ARP 設定狀態、訓練資料和快照從來源磁碟區傳送到目標磁碟區。當 SnapMirror 目標磁碟區轉換為讀寫模式時，必須在轉換後明確地在目標磁碟區上啟用 ARP 的學習模式。
...
升級已使用 ARP 的 ONTAP 叢集時，除了link:../upgrade/special-considerations.html["您必須解決的 ONTAP 特殊注意事項"]之外，您還應該注意升級導致的 ARP 特定行為變化。



[[arp-default-enablement-after-upgrading-to-ontap-9-18-1]]
=== 升級至 ONTAP 9.18.1 後的 ARP 預設啟用

從 ONTAP 9.18.1 開始，ARP/AI 預設啟用功能適用於 AFF A 系列、AFF C 系列、ASA 和 ASA r2 系統。升級後，所有新磁碟區在 12 小時寬限期後將自動啟用 ARP/AI。您必須link:index.html#manual-default-enablement["手動啟用 ARP"]在現有磁碟區上。



[[snapmirror-relationships-after-upgrading-to-ontap-9-19-1]]
=== SnapMirror 升級至 ONTAP 9.19.1 後的關係

升級後，如果對處於 SnapMirror 同步或 SnapMirror 活動同步關係中的磁碟區啟用 ARP，則該關係會短暫進入不同步狀態。這種情況是暫時的，關係會自動重新同步，無需人工幹預。



[[flexgroup-volumes-after-upgrade]]
=== 升級後的 FlexGroup Volume

ONTAP 9.16.1 和 9.17.1 不支援 FlexGroup 磁碟區的 ARP/AI。升級到這些版本後、已啟用 ARP 的 FlexGroup 磁碟區將繼續使用 ARP/AI 之前的 ARP 模型。從 ONTAP 9.18.1 開始、FlexGroup 磁碟區將使用 ARP/AI 模型。
