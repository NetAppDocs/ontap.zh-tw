...
* ARP/AI 分析在 SnapMirror 同步和 SnapMirror 主動同步 SAN 關係中的作用中讀寫 Volume 上執行。ARP 會在主要 Volume 上建立快照，並執行以機器學習為基礎的勒索軟體偵測。
* 在 SnapMirror 同步或 SnapMirror 主動同步關係中，ARP 快照不會複製到次要 Volume。故障轉移後，您需要在恢復 Volume 上 link:enable-task.html["再次啟用 ARP"]。
