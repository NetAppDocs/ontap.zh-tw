...
從ONTAP 9.12.1 開始，系統管理器與NetApp控制台整合。透過控制台，管理員可以從單一控制平面管理混合多雲基礎設施，同時保留熟悉的系統管理器儀表板。登入系統管理員時，管理員可以選擇存取NetApp控制台中的系統管理器介面或直接存取系統管理員。詳細了解xref:../concepts/sysmgr-integration-console-concept.html[System Manager 與NetApp Console 集成] 。
