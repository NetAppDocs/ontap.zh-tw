...
| SnapMirror Business Continuity （ SM-BC ）增強功能  a| 
* link:../smbc/smbc_admin_add_and_remove_volumes_in_consistency_groups.html["在不中斷營運的情況下、將磁碟區新增至一致性群組、並建立有效的 SM-BC 關係。"]
* link:../smbc/supported-configurations-reference.html#ndmp-restore["使用 NDMP 還原搭配 SM-BC"]。
...
| xref:../concepts/rdma-concept[擴充硬體支援 RDMA 叢集互連] | ONTAP 支援 AFF A900 、 ASA A900 和 FAS9500 系統、用於叢集互連 RDMA 與 X91153A 叢集 NIC 、有助於減少延遲、縮短容錯移轉時間、並加速節點之間的通訊。 
...
* xref:../use-cases-restrictions-concept.html#multi-admin-verification-with-volumes-protected-with-arp[多管理驗證功能與自主勒索軟體保護]
