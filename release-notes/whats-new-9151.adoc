...
xref:../smb-admin/windows-backup-symlinks.html[伺服器上的 Windows 備份應用程式和 Unix 風格的符號鏈接]
 a| 
當 Windows 備份應用程式遇到 Unix 風格的符號連結（symlink）時，ONTAP 會追蹤該連結並傳回實際資料進行備份。從 ONTAP 9.15.1 開始，您也可以選擇備份符號連結本身，而不是它指向的資料。這可以帶來許多好處，例如提升備份應用程式的效能。您可以使用 ONTAP CLI 或 ONTAP REST API 啟用此功能。
...
使用 SnapMirror 非同步保護的一致性群組現在支援一致性群組中最多 80 個磁碟區。
 ...
 a| 
xref:../consistency-groups/configure-task.html[支援一致性群組的 ONTAP REST API 和 ONTAP CLI 操作的管理員權限等級]
 a| 
現在，管理權限等級支援一致性群組的 ONTAP CLI 和 ONTAP REST API 操作。
...
xref:../concepts/ontap-and-vmware.html[使用 Windows Server 容錯移轉叢集持續保留 VMware 虛擬磁碟區]
 a| 
ONTAP 目前支援 VMware 虛擬 Volume（vVols）以及使用傳統 LUN 的持續保留。從 ONTAP 9.15.1 開始，您也可以使用 vVol 建立持續保留。此功能在 ONTAP tools for VMware vSphere 9 中實作。它僅在 Windows Server 容錯移轉叢集（WSFC）中受支援，這是一組叢集化的 Windows 虛擬機器。
