...
* xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[多管理驗證功能與自主勒索軟體保護]
...
從ONTAP 9.12.1 開始，系統管理器與NetApp控制台整合。詳細了解xref:../concepts/sysmgr-integration-console-concept.html[System Manager 與NetApp Console 集成] 。
