...
| 自主勒索軟體防護 (ARP/AI) | 在支援的平台上，所有新磁碟區在 12 小時寬限期後預設自動啟用 ARP/AI。 | ONTAP 9.18.1 
...
| ONTAP SVM 快取正向憑證的生存時間 (TTL) | 從 ONTAP 9.17.1 開始，ONTAP SVM 快取正向憑證的存留時間 （TTL） 已從 24 小時 （86,400,000 毫秒） 減少至 1 小時 （3,600,000 毫秒）。 | ONTAP 9.17.1 
...
| 負載共享鏡像 | 建立負載共享鏡像關係時，SVM 不能啟用儲存限制。 | ONTAP 9.16.1. 
...
| SnapMirror 雲端 | SnapMirror 雲端將支援的 S3 儲存桶數量從 6 個增加到 100 個。 | ONTAP 9.19.1 


| S3 連線流量控制限制 | 對於記憶體小於等於 32 GB 的雲端執行個體，ONTAP S3 連線的流量控制限制已從 2 MB 降低至 256 KB。 | ONTAP 9.19.1 


| S3 閒置連線逾時 | S3 空閒連線逾時時間已從 20 分鐘減少至 5 分鐘。 | ONTAP 9.19.1 
...
| 叢集對等 | 將支援的叢集對等節點限制從 255 增加到 400，提高了具有大量叢集對等關係的環境的擴充性。 | ONTAP 9.14.1. 
...
* FlexGroup 中的構成要素最大數量不能超過 24 個。
* SVM-DR Volume 限制為每個節點 500，每個 HA 配對 1000（包括 FlexVol Volume 和 FlexGroup 成分）。
+

NOTE: 限制條件因平台而異。您應該前往 link:https://hwu.netapp.com["NetApp Hardware Universe"^] 並選取您的平台和 ONTAP 版本，以驗證組態的正確限制。
...
| SnapMirror 並行傳輸 | 非同步卷級並發傳輸的最大數量已從 100 增加到 200。高階系統上的SnapMirror雲端傳輸次數從 32 次增加到 100 次，低階系統上的SnapMirror傳輸次數從 6 次增加到 20 次。 | 部分9.8 ONTAP 
