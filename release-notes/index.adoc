...
* 瞭解 https://docs.netapp.com/us-en/ontap-afx/release-notes/whats-new-9191.html["適用於NetApp AFX 儲存系統的全新增強型ONTAP軟體功能"^]。
* 瞭解 https://docs.netapp.com/us-en/asa-r2/release-notes/whats-new-9191.html["適用於NetApp ASA r2 儲存系統的全新增強型ONTAP軟體功能"^]。
...
ONTAP 9.19.1 在安全性、擴充性和儲存管理方面提供了一系列新增和增強功能，以支援資料基礎架構現代化。這些增強功能有助於您擴展規模、提高營運效率並保護資料，以因應未來挑戰。如需新增功能和增強功能的完整清單，請參閱 xref:whats-new-9191.adoc[ONTAP 9.19.1 的新增功能]。

* xref:../performance-admin/guarantee-throughput-qos-task.html[可設定的 QoS 突發原則]
+
可設定的 QoS 突發原則允許在 QoS 限制之外進行可控且有時限的效能突發，從而在尖峰事件期間提高工作負載回應能力，而不會犧牲公平性或需要過度配置。

* xref:../nfs-admin/enable-nfs-over-tls-task.html[透過 TLS 1.3 的 NFS]
+
使用基於標準的 TLS 1.3 加密保護傳輸中的 NFS 資料，有助於加強保護並簡化企業級加密 NFS 的採用。 

* xref:../s3-config/regenerate-access-keys.html[每個用戶擁有多個 S3 存取金鑰]
+
新增對每位使用者多個作用中 S3 存取金鑰的支援，實現無縫認證資料輪換，無需應用程式停機時間，協助客戶在維持對物件工作負載不間斷存取的同時，強化安全性與法規遵循。

* xref:../snapmirror-active-sync/protect-nas-task.html[適用於 NAS 的 SnapMirror active sync]
+
借助 NAS 支援，SnapMirror active sync 現在是一個統一的業務連續性解決方案，可實現零 RPO 與透明的故障轉移。 

* xref:../san-config/configure-fc-nvme-hosts-ha-pairs-reference.html[FC 直接連接]
+
引入安全的 FC HBA 卡支援，實現可信任主機連線功能，為硬體支援的機上 FC 安全性奠定基礎，同時保持效能並滿足企業法規遵循要求。 

* xref:../san-admin/multipath-automatic-lif-failover-support.html[統一的雙主動式 LIF 故障轉移]
+
AFF 系統提供雙主動式多重路徑與自動 LIF 容錯移轉功能，讓所有 SAN 路徑保持最佳化的雙主動式狀態，確保主機 I/O 在容錯移轉期間能立即持續運作。 

* link:https://docs.netapp.com/us-en/ontap-fli/get-started/learn-about-data-migration.html["FLI 中的 iSCSI 支援"^]
+
為外部 LUN 匯入 (FLI) 新增 iSCSI 支援，讓使用者能夠使用現有的 iSCSI 連線功能，以最小的中斷將區塊資料從第三方儲存系統移轉至 ONTAP。 

* xref:../s3-config/create-modify-bucket-policy-task.html[S3 條件寫入與刪除]
+
啟用 S3 API 條件寫入和刪除的支援，允許應用程式在物件更新和移除期間強制執行資料一致性檢查，從而降低並行和分散式物件工作負載中意外覆寫或刪除的風險。

* xref:../concepts/path-failover-concept.html#nas-path-failover[聯合 LIF 故障轉移]
+
AFF 和 ASA 系統支援 iSCSI 協定 LIF 在雙主動式和 active-active-scsi-only 多重路徑組態中的自動 LIF 容錯移轉，以及 NVMe/TCP 協定 LIF 在雙主動式多重路徑組態中的自動 LIF 容錯移轉。 

* xref:../networking/improve-tcp-wan-performance.html[增強的 WAN 效能]
+
TCP RACK 增強的 WAN 效能可提升壅塞網路的處理量和恢復能力，加速遠端檔案存取和站點間資料服務。 

* xref:../networking/configure-tls-offload.html[硬體加速的 kTLS 可加快後端叢集加密]
+
透過引入硬體加速的 kTLS 來增強後端叢集加密，將加密處理卸載到支援的 SmartNIC，從而提供更高的效能和更低的 CPU 例行成本，同時保持叢集和 HA 流量始終在線的安全通訊。 

* xref:../anti-ransomware/use-cases-restrictions-concept.html#snapmirror[針對同步複寫工作負載的 AI 驅動勒索軟體偵測]
+
將自主勒索軟體防護 (ARP/AI) 擴展到 SnapMirror 同步同步 （SAN 和 NAS） 和 SnapMirror 雙主動式 SAN 環境，在零 RPO 架構中保留偵測和恢復訊號。 

* xref:../concept-svm-admin-dashboard-sm.html[使用新的 ONTAP System Manager 儀表板監控和管理 SVM]
+
提供租戶友善的 SVM 管理 GUI，無需複雜的 CLI 介面。 
...
ONTAP 9.18.1 在安全性、可擴充性和儲存管理方面提供了新的和增強的功能，以支援資料基礎架構現代化。這些增強功能可協助您擴展規模、提高營運效率並確保未來資料安全。如需查看所有新增功能和改進的完整列表，請參閱xref:whats-new-9181.adoc[ONTAP 9.18.1 的新增功能]。

* xref:../networking/configure-backend-encryption.html[後端叢集高可用性加密]
+
能夠使用 mTLS 對後端叢集網路進行加密。

* xref:../networking/configure_network_security_using_federal_information_processing_standards_fips.html[PQC對傳輸中資料的支持]
+
能夠使用後量子演算法處理傳輸中的資料。

* xref:../networking/ipsec-prepare.html[支援 IPv6 IPSec 和硬體卸載]
+
支援使用 IPv6 位址進行硬體卸載的 IPSec。

* xref:../anti-ransomware/use-cases-restrictions-concept.html#supported-configurations[ARP/AI 支援FlexGroup卷]
+
NetApp FlexGroup磁碟區支援 ARP/AI，提供基於機器學習的進階勒索軟體偵測和保護。

* xref:../san-admin/nvme-copy-offload.html[NVMe 的複製卸載]
+
透過將複製操作直接卸載到儲存設備來加速大數據傳輸，從而降低主機 CPU 和網路負載。

* xref:../s3-snapshots/list-restore-s3-snapshot.html[儲存驅動的 S3 儲存桶快照恢復]
+
...
+
ARP/AI 支援具備加密型異常偵測功能的 SAN Volume，引入用於詳細熵統計資料的新命令，並將先前主要針對 NAS 的勒索軟體保護郵件統一整合至 System Manager 中。可設定的偵測閾值與更具確定性的快照保留，為各種工作負載提供更大的靈活度。
...
+
ONTAP 9.16.1 為 MAV 架構新增了更多命令，以增強對惡意內部人員的保護。這些增強功能包括許多 Consistency Group （CG）、VScan 和 Autonomous Ransomware Protection （ARP/AI） 管理命令，以及 NVMe 組態命令。

* xref:../anti-ransomware/index.html[利用 AI 增強功能（ ARP/AI ）提供自主勒索軟體保護]
+
Autonomous Ransomware Protection 已升級全新的 AI 功能，使其能夠以 99% 的精準率與召回率偵測並回應勒索軟體攻擊。由於 AI 是在完整的資料集上進行訓練，因此在 FlexVol® Volume 上執行的 ARP/AI 不再需要學習期，且 ARP/AI 會立即以作用中模式啟動。ARP/AI 還引進了獨立於 ONTAP 升級之外的自動更新功能，以確保針對最新威脅提供持續的保護與彈性。
...
+
從 ONTAP 9.15.1 開始，您也可以選擇備份符號連結本身，而不是它指向的資料。這可以帶來許多好處，包括提升備份應用程式的效能。您可以使用 ONTAP CLI 或 ONTAP REST API 啟用此功能。
...
** xref:../anti-ransomware/support-arpai.html#multi-admin-verification-with-arp-ai-protected-volumes[多重管理驗證支援]
+
...
+
如果 FlexGroup 成員節點容量失衡，FlexGroup 可以透過 ONTAP CLI、ONTAP REST API 和 System Manager 進行無中斷的重新平衡和管理。為了獲得最佳效能，FlexGroup 內的成員節點已使用容量應均勻分佈。
