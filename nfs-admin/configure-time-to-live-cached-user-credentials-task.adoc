...
summary: 您可以透過修改儲存虛擬機器的 NFS 伺服器來設定 ONTAP 在其內部快取中儲存 NFS 使用者憑證的時間長度。這有助於緩解因名稱伺服器負載過高或憑證變更而影響 NFS 使用者存取的某些問題。 
---
...
TTL 以毫秒為單位。從 ONTAP 9.17.1 及更新版本開始，預設值為 1 小時（3,600,000 毫秒）。在 ONTAP 9.16.1 及更早版本中，預設值為 24 小時（86,400,000 毫秒）。此值的允許範圍為 1 分鐘（60,000 毫秒）至 7 天（604,800,000 毫秒）。
