...
NOTE: 了解啟用擴充組的 AUTH_SYS 驗證先決條件(`-auth-sys-extended-groups`) 使用超出預設最大 16 個群組 ID，請參閱link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/What_are_the_prerequisites_for_enabling_auth_sys_extended_groups#["NetApp知識庫：啟用 auth-sys-extended-groups 的先決條件是什麼？"^]
...
----
.相關資訊
* link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/How_does_AUTH_SYS_Extended_Groups_change_NFS_authentication["NetApp知識庫：針對ONTAP 9 的 NFS 驗證的 AUTH_SYS 擴充組更改"^]
