...
====
* 用戶端必須支援基於 TCP 的 RPC with TLS，必須設定為在掛載時請求 TLS，並且必須信任簽署 NFS 伺服器憑證的憑證授權單位。用戶端組態步驟因用戶端而異，因此此處未作說明。如需 Linux 用戶端的端對端逐步說明，請參閱 link:https://docs.netapp.com/us-en/ontap-technical-reports/nfs-tls/nfs-tls-configure.html["設定 NFS over TLS"]。
