...
* 當您使用 iSCSI 協定設定 ONTAP Mediator 時，您可以使用任何版本的 ONTAP Mediator 來監控 MetroCluster IP 組態。
* 從 ONTAP 9.19.1 和 ONTAP Mediator 1.12 開始、您可以設定使用 HTTPS 進行通訊的 ONTAP Mediator 來監控您的 MetroCluster IP 組態。
...
在升級或安裝 ONTAP Mediator 之前，請先查看以下資訊。
...
* 當您升級到包含 SCST 套件的新 ONTAP Mediator 版本時，安裝程式會自動升級到建議的 SCST 版本，除非有更高版本可用。有關手動安裝更高 SCST 版本的說明，請參閱link:manage-task.html["管理 ONTAP Mediator"]。有關支援的版本，請參閱link:whats-new-concept.html#scst-support-matrix["SCST 支援對照表"]。
...
* 從ONTAP Mediator 1.11 開始， MetroCluster IP 設定支援 IPv6。
...
=== ONTAP Mediator 的 HTTPS 和 iSCSI 安裝選項

* 在 ONTAP Mediator 中，SCST 套件用於提供 iSCSI，以進行 MetroCluster IP 組態中的通訊。SCST 套件是在安裝期間編譯的核心模組。從 ONTAP 9.19.1 和 ONTAP Mediator 1.12 開始，您可以設定 ONTAP Mediator 使用 HTTPS 而非 iSCSI 來監控 MetroCluster IP 組態。
...
支援iSCSI目標ONTAP
 a| 
（ MetroCluster IP 設定必需）信箱的 iSCSI 資料連接

|===

NOTE: 對於使用 HTTPS 進行通訊的 SnapMirror active sync 或 MetroCluster IP ONTAP Mediator，ONTAP 不需要啟用或連接連接埠 3260。
...
+
** 相容：10.1 和 9.7 ^1^
** 建議：10.2、10.0、9.8、9.6、9.4 和 8.10
...
使用 ONTAP Mediator 安裝 SCST 套件時，適用下列要求。


NOTE: 如果您安裝的是僅支援 HTTPS 的 ONTAP Mediator 1.12 或更高版本，則不會安裝 SCST 套件，因此核心相容性需求不適用。
...
====
* 核心版本必須符合作業系統版本。
* 請勿將核心升級至 ONTAP Mediator 版本支援的作業系統版本以上，否則經過測試的 SCST 模組可能無法正常運作。
...
* 如果您安裝的是僅支援 HTTPS 的 ONTAP Mediator 1.12 或更高版本，則不會安裝 SCST 套件，您可以跳過此工作。
