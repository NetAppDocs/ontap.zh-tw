...
* 您可以使用任何版本的 ONTAP Mediator 來監控 MetroCluster IP 組態。
...
在升級或安裝 ONTAP Mediator 之前，請先查看以下幾點。
...
* 升級到新的 ONTAP Mediator 版本時，安裝程式會自動升級到建議的 SCST 版本，除非有更高版本可用。有關手動安裝更高 SCST 版本的說明，請參閱link:manage-task.html["管理 ONTAP Mediator"]。有關支援的版本，請參閱link:whats-new-concept.html#scst-support-matrix["SCST 支援對照表"]。
...
.網路需求
請確保以下連接埠可供 ONTAP Mediator 使用且未被佔用：
...
|===
對於 SMBC 客戶而言，ONTAP 不需要啟用或連線連接埠 3260。

* 如果使用第三方防火牆，請參閱 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#firewall-requirements-for-ontap-mediator["ONTAP Mediator 的防火牆需求"^]。
...
====
* 核心版本必須符合作業系統版本。
* 請勿將核心升級至 ONTAP Mediator 版本支援的作業系統版本以上，因為測試過的 SCST 模組可能無法正常運作。
