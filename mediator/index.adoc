...
* 當您升級到包含 SCST 套件的新 ONTAP Mediator 版本時，安裝程式會自動升級到建議的 SCST 版本，除非有更高版本可用。有關手動安裝更高 SCST 版本的說明，請參閱 link:manage-task.html["管理 ONTAP Mediator"]。有關支援的版本，請參閱 link:whats-new-concept.html#scst-support-matrix["SCST 支援對照表"]。
...
* 在 ONTAP Mediator 中，SCST 套件用於為 MetroCluster IP 組態中的通訊提供 iSCSI 功能。SCST 套件是一個核心模組，在安裝過程中編譯。從 ONTAP Mediator 1.12 開始，您可以設定 ONTAP Mediator 使用 HTTPS 而不是 iSCSI 來監控 MetroCluster IP 組態。


[CAUTION]
====
* 當 ONTAP Mediator 在 MetroCluster IP 配置中使用 HTTPS 協定時，單一 ONTAP Mediator 執行個體只能監控一個 MetroCluster IP 配置。不支援同時監控多個 MetroCluster IP 配置。
