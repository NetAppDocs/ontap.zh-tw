...
ONTAP Mediator 依賴 SCST 提供 iSCSI（請參閱 http://scst.sourceforge.net/index.html[]）。該軟體包是一個核心模組，在安裝過程中專門針對核心進行編譯。核心的任何更新都可能需要重新安裝 SCST。或者，您可以解除安裝 ONTAP Mediator，然後重新安裝，最後重新設定 ONTAP 關係。


NOTE: 伺服器作業系統核心的任何更新都應與 ONTAP 中的維護視窗協調進行。
