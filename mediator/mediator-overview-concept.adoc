...
從 ONTAP 9.19.1 和 ONTAP Mediator 1.12 開始，您可以設定使用 HTTPS 或 iSCSI 來監控 MetroCluster IP 組態的 ONTAP Mediator。您可以選擇安裝同時支援 iSCSI 和 HTTPS 的 ONTAP Mediator，或僅支援 HTTPS 的 ONTAP Mediator。如果安裝的 ONTAP Mediator 僅支援 HTTPS，則不會安裝 SCST 套件。如需詳細資訊，請參閱 link:index.html#https-iscsi-installation["ONTAP Mediator 的 HTTPS 和 iSCSI 安裝選項"]。
