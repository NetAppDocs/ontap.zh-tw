...
* 支援在 MetroCluster IP 組態中設定使用 HTTPS 進行通訊的 ONTAP Mediator。HTTPS 透過使用 SSL/TLS 加密整個工作階段來提高安全性。您可以選擇安裝僅使用 HTTPS 通訊協定的 ONTAP Mediator。如果使用此安裝方法，則不會安裝 SCST 套件。link:index.html#https-iscsi-installation["深入瞭解 ONTAP Mediator 的 HTTPS 和 iSCSI 安裝選項"]。
