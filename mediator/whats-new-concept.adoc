...
* 支援 Rocky Linux 10.2、9.8 和 8.10。
* 支援 Oracle Linux 10.1 和 9.8，同時支援 rhck 和 uek 核心。
* 從 ONTAP Mediator 1.12.1 和 ONTAP 9.19.1 開始，設定了 HTTPS 的單一 ONTAP Mediator 執行個體可以同時監控以下組態：
+
** 如果安裝在配備 8 核心 CPU 和至少 16GB RAM 的主機上，最多可安裝 10 個四節點或 5 個八節點 MetroCluster IP 組態。請參閱link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/concept_mediator_requirements.html#mediator-multiple-mccip["使用 ONTAP Mediator 監控多個 MetroCluster IP 組態"^]。
** 如果安裝在具有 2 核心 CPU 的主機上，則一個四節點 MetroCluster IP 組態和一個 SnapMirror 主動同步對等關係。請參閱 link:../snapmirror-active-sync/mediator-install-task.html#ontap-mediator["為SnapMirror主動同步配置ONTAP調解器"]。


* 包含一個用於修理損壞的 SCST 安裝的新工具。安裝 ONTAP Mediator 後，您可以在 `/usr/local/bin/mediator_repair_scst`找到該工具。如果升級後的作業系統需要預設 SCST 套件中未提供的 SCST 修補程式，則可以在作業系統升級後使用該工具。
...
* 支援 Rocky Linux 10.2、9.8 和 8.10。
* 支援 Oracle Linux 10.1 和 9.7，同時支援 rhck 和 uek 核心。
* 從 ONTAP 9.19.1 和 ONTAP Mediator 1.12 開始，您可以設定在 MetroCluster IP 組態中使用 HTTPS 進行通訊的 ONTAP Mediator。HTTPS 透過使用 SSL/TLS 加密整個工作階段來提高安全性。您可以選擇安裝僅支援 HTTPS 的 ONTAP Mediator。如果使用此安裝方法，則不會安裝 SCST 套件。link:index.html#https-iscsi-installation["深入瞭解 ONTAP Mediator 的 HTTPS 和 iSCSI 安裝選項"]。
...
1.11
 a| 
* 支援 RHEL 10.1、10.0、9.7、9.6、9.4 和 8.10。
* 支援 Rocky Linux 10.1、9.7 和 8.10。
* 支援 Oracle Linux 10.0 和 9.6，同時支援 rhck 和 uek 核心。
* 為MetroCluster IP 配置新增對 IPv6 的支援。
* 新增了對 fapolicyd 的支援。
...
1.10
 a| 
* 支援 RHEL 10.0、9.6、9.4 和 8.10。
* 支援 Rocky Linux 10.0、9.6 和 8.10。
* 將 Python 基礎版本從 Python 3.9 升級到 Python 3.12。
...
| 適用於 ONTAP Mediator 的作業系統 | 1.12.1 | 1.12 | 1.11 | 1.10 | 1.9.1 | 1.9 | 1.8 | 1.1.7 | 1.6. | 1.5. | 1.4. | 1.3. | 1.2. | 1.1. | 1.0 
