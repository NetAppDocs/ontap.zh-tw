...
設定 Mediator 服務
 a| 
執行中的步驟link:../task_metrocluster_configure.html#configure-the-ontap-mediator-service["設定ONTAP 「BMC」服務"]。
...
檢查 Mediator 的健全狀況
 a| 
執行中的步驟link:../task_metrocluster_troubleshooting.html["疑難排解IP MetroCluster 位址組態的問題"]。
...
執行切換和切換
 a| 
執行中的步驟link:../task_metrocluster_switchover_switchback.html["執行IP MetroCluster 位址切換和切換"]。
