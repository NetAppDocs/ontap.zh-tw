...
NOTE: 如果您安裝的是僅支援 HTTPS 的 ONTAP Mediator 1.12 或更高版本，則 SCST 軟體包未安裝，主機作業系統核心可以升級而不會影響 ONTAP Mediator。有關升級說明，請參閱主機作業系統文件。
...
.關於這項工作
* 在安裝ONTAP調解器之前執行此步驟。如果您使用的 SCST 版本比ONTAP Mediator 安裝程式捆綁的版本新，則安裝程式將跳過此步驟。
* 在某些情況下，由於升級後的作業系統需要修補程式，因此在升級後的作業系統上預設安裝 SCST 可能會失敗。
+
從 ONTAP Mediator 1.12.1 開始，如果安裝失敗，您可以使用位於 `/usr/local/bin/mediator_repair_scst`的 SCST 修理工具來修理安裝。

+
如果升級後的作業系統需要修補程式，則該工具包含適用於 ONTAP Mediator 套件中所提供 SCST 版本的修補程式。該 `mediator_repair_scst`工具是一個獨立式工具，與主安裝程式分隔，並封裝在 ONTAP Mediator `tgz`檔案中。它不會影響 ONTAP Mediator 的安裝或升級路徑。
