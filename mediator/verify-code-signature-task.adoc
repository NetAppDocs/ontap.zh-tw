...
[NOTE]
====
* 自 2025 年 6 月 15 日起，您將無法安裝或升級至 ONTAP Mediator 1.9 和 1.8，因為代碼簽署驗證憑證已過期。請改為安裝或升級至 ONTAP Mediator 1.11 或 1.10。
...
----
. 使用關聯的憑證驗證  `ontap-mediator-1.11.0.sig.tsr` 和  `ontap-mediator-1.11.0.tsr` 檔案：
+
[role="tabbed-block"]
====
.OpenSSL 3.x
