...
summary: 若要安裝或升級 ONTAP Mediator、您必須取得安裝套件並在主機上執行安裝程式。 
---
= 安裝ONTAP 《不知道如何安裝》套件
:allow-uri-read: 
:icons: font
...
[role="lead"]
若要安裝或升級 ONTAP Mediator、您必須取得安裝套件並在主機上執行安裝程式。

.關於這項工作
* 從 ONTAP 9.19.1 和 ONTAP Mediator 1.12 開始，您可以選擇安裝支援 iSCSI 和 HTTPS 的 ONTAP Mediator，或僅支援 HTTPS 的 ONTAP Mediator。如果您選擇僅安裝支援 HTTPS 的 ONTAP Mediator，則不會安裝 SCST 軟體套件。
...
* 在安裝或升級至 ONTAP Mediator 1.12 或更高版本時，安裝程式會提示您回答以下問題：
+
...
+
** 如果您打算使用已設定了 iSCSI 的 ONTAP Mediator 來監控一個或多個 MetroCluster IP 配置，請回覆 `Yes`。
** 如果您計劃僅使用 ONTAP Mediator 進行 SnapMirror 主動同步，或計劃使用 HTTPS 設定 ONTAP Mediator 來監控一個或多個 MetroCluster IP 組態，請回應。 `No`
+
或者，您可以在 `./ontap-mediator-1.12.1/ontap-mediator-1.12.1` 指令中使用 `--https-only` 選項來指定僅使用 HTTPS。



* 對於自動化使用案例，您可以使用  `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 -y` 命令來跳過安裝程式提示。
...
[NOTE]
====
若要在安裝期間跳過簽章檢查，請使用此命令： `./ontap-mediator-1.12.1/ontap-mediator-1.12.1 --skip-code-signature-check`

====
+
安裝程式會建立所需的帳戶並安裝所需的軟體包。如果 Mediator 已安裝，則會提示您進行升級。



.支援 HTTPS 和 iSCSI 的 ONTAP Mediator 安裝範例（主控台輸出）
[%collapsible]
====
[listing]
----
...
----
====
.支援 HTTPS 和 iSCSI 的 ONTAP Mediator 升級範例（主控台輸出）
[%collapsible]
====
[listing]
----
...
----
====
.僅支援 HTTPS 的 ONTAP Mediator 安裝範例（主控台輸出）
[%collapsible]
====
[listing]
----
...
----
====
.僅支援 HTTPS 的 ONTAP Mediator 升級範例（主控台輸出）
[%collapsible]
====
[listing]
----
...
從ONTAP Mediator 1.4 開始，UEFI 系統上啟用了安全啟動機制。啟用安全啟動後，安裝完成後必須採取額外步驟來註冊安全金鑰。


NOTE: 如果您安裝的是僅支援 HTTPS 的 ONTAP Mediator 1.12 或更高版本，則不會安裝 SCST 套件，您可以跳過此工作。
...
NOTE: 安裝完成後，系統輸出會提供 README 檔案和金鑰位置。
...
[NOTE]
====
您可以將私鑰保留在預設位置，也可以將其移動到安全位置。您必須將公鑰保存在現有位置，以便啟動管理器可以使用它。更多資訊請參閱 README.module-signing 檔案：

`[root@hostname ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv`

====
. 重新啟動主機，並使用裝置的 UEFI 啟動管理員批准新的 MOK。您需要使用提供的密碼短語。 `mokutil`實用性link:index.html#install-ontap-mediator-when-uefi-secure-boot-is-enabled["啟用 UEFI 安全開機時，請安裝 ONTAP Mediator"]。
...
安裝ONTAP Mediator 後，如果 systemctl 狀態 `mediator-scst`顯示為失敗（非活動狀態），請依照下列步驟對 SCST 核心模組進行簽署。


NOTE: 如果您安裝的是僅支援 HTTPS 的 ONTAP Mediator 1.12 或更高版本，則不會安裝 SCST 套件，您可以跳過此工作。

.步驟
. 在建置過程中，會產生一對公鑰/私鑰。 `/opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/`使用以下命令進入目錄：
+
`[root@mediator-host ~]# ls /opt/netapp/lib/ontap_mediator/ontap_mediator/SCST_mod_keys/
README.module-signing  scst_module_key.der  scst_module_key.priv
[root@mediator-host ~]#`

. 執行以下命令，開始將公鑰匯入UEFI金鑰庫的過程：
+
...
. mokutil 軟體在匯入過程中會要求為此金鑰設定一個臨時密碼。
. 確認導入過程是否已開始 `mokutil --list-new`然後重啟系統。引導程式啟動 EFI MOK 管理器。
. 使用螢幕上的選單開啟 SCST 核心模組金鑰。啟動後，運行 `systemctl status mediator-scst`。服務啟動後，SCST 核心模組將被簽署。
