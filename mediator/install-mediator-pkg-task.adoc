...
[CAUTION]
====
* 在 MetroCluster IP 組態中，當 ONTAP Mediator 設定為使用 HTTPS 通訊協定時，單一 ONTAP Mediator 執行個體只能監控一個 MetroCluster IP 組態。不支援同時監控多個 MetroCluster IP 組態。
...
** 如果您計劃僅使用 ONTAP Mediator 進行 SnapMirror 主動同步，或者如果您計劃使用 HTTPS 設定 ONTAP Mediator 以監控每個 ONTAP Mediator 執行個體的單一 MetroCluster IP 組態，請回應 `No`。
+
或者，您可以在 `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y` 指令中使用 `--https-only` 選項來指定僅使用 HTTPS。
...
[NOTE]
====
若要在安裝期間跳過簽章檢查，請使用此命令： `./ontap-mediator-1.12.0/ontap-mediator-1.12.0 -y --skip-code-signature-check`
