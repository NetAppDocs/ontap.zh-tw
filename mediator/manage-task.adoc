...
[NOTE]
====
* 在 ONTAP Mediator 1.8 及更早版本中，使用的 CA 檔案包括： `ca.key`，， `ca.csr` `ca.srl`和 `ca.crt`。從 ONTAP Mediator 1.9 開始，所需的檔案為 `intermediate.key`， `intermediate.csr`， `intermediate.srl`和 `intermediate.crt`。
