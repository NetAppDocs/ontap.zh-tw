...
NOTE: 對於使用設定為 HTTPS 的 ONTAP Mediator 的 SnapMirror 業務連續性（SM-BC）叢集和 MetroCluster IP 組態，您必須將  `intermediate.crt` 和  `root_ca.crt` 憑證新增至 ONTAP 叢集。請參閱 link:../snapmirror-active-sync/mediator-install-task.html["為 SnapMirror 主動同步配置 ONTAP 調解器和集群"] 和 link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/task_configuring_the_ontap_mediator_service_from_a_metrocluster_ip_configuration.html["設定 ONTAP Mediator 以進行 MetroCluster IP 組態"^]。
