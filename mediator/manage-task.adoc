...
NOTE: 對於 SnapMirror Business Continuity （ SM-BC ）叢集、您必須將和憑證新增 `intermediate.crt` `root_ca.crt` 至 ONTAP 叢集。請參閱。 link:../snapmirror-active-sync/mediator-install-task.html["設定 ONTAP Mediator 和叢集以進行 SnapMirror 主動式同步"]
