...
* SVM 根磁碟區的預設匯出原則必須包含一條規則，允許所有用戶端透過 NFS 進行開放存取。
+
深入瞭解 link:../nfs-config/open-export-policy-svm-root-volume-task.html["ONTAP SVM 上的 NFS 用戶端存取。"]

* 如果您建立了自訂匯出原則，則必須將相同的規則新增至預設原則。
+
深入瞭解 link:../nfs-admin/default-export-policy-svms-concept.html["ONTAP NFS SVM 的預設匯出原則。"]
