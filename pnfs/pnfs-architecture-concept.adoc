...
NOTE: 這些命令中不包含磁碟區名稱。而是使用與這些磁碟區關聯的數字 ID：MSID 和 DSID。若要尋找與映射關聯的磁碟區，您可以在 ONTAP CLI 的進階權限中使用 `volume show -dsid [dsid_numeric]`或 `volume show -msid [msid_numeric]`。
...
* https://www.netapp.com/pdf.html?item=/media/12385-tr4571.pdf["NetApp技術報告 4571： FlexGroup最佳實踐"^]
