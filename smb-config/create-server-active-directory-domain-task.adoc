...
* 從 ONTAP 9.13.1 開始，您可以為 SMB 伺服器設定預設站台。當 LIF 沒有站台成員資格時，ONTAP 可以使用此站台；如果在尋找受信任網域的網域控制器時，受信任網域的站台探索失敗，ONTAP 也可以使用此站台作為備用站台。
...
. 在 AD 網域中建立 SMB 伺服器： `+vserver cifs create -vserver vserver_name -cifs-server smb_server_name -domain FQDN [-ou organizational_unit][-default-site site_name][-netbios-aliases NetBIOS_name, ...][-keytab-uri {(ftp|http)://hostname|IP_address}][-comment text]+`
+
...
從 ONTAP 9.13.1 開始，如果 ONTAP 無法探索受信任網域的站台資訊，則 SMB 伺服器預設站台也可用作備用站台。
