...
[role="lead"]
如果啟用了 SMB 匯出原則規則、則 SVM 根磁碟區的預設匯出原則必須包含一條規則、允許所有用戶端透過 SMB 進行開放式存取。如果沒有這樣的規則、所有 SMB 用戶端都將被拒絕存取 SVM 及其磁碟區。匯出原則規則預設為停用狀態。
...
.相關資訊
link:../smb-admin/enable-disable-export-policies-task.html["了解如何啟用或停用 ONTAP 匯出原則以進行 SMB 存取"].
