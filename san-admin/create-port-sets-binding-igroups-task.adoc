...
summary: 除了使用 Selective LUN Map 之外，您還可以建立連接埠集並將連接埠集繫結至 igroup，以進一步限制啟動器可用於存取 LUN 的 LIF。如果您未將連接埠集繫結至 igroup，則 igroup 中的所有啟動器都可以透過擁有 LUN 的節點及擁有節點的 HA 合作夥伴上的所有 LIF 來存取對應的 LUN。 
---
= 建立連接埠集並繫結至igroup
:allow-uri-read: 
:icons: font
