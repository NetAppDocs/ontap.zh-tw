...
summary: Microsoft Offloaded Data Transfer（也稱為 copy offload）可在儲存裝置內或相容的儲存裝置之間直接傳輸資料、而無需透過主機電腦傳輸資料。 
---
= Microsoft卸載資料傳輸（ODX）
:allow-uri-read: 
:icons: font
...
在虛擬化環境中使用 VMware 和 Microsoft 複製卸載時，您的 LUN 必須對齊。未對齊的 LUN 會降低效能。link:io-misalignments-properly-aligned-luns-concept.html["了解更多關於未對齊 LUN 的信息"]。
