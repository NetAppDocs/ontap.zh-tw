...
[role="lead"]
從ONTAP 推出支援NVM9.4的NVMe/FC開始、支援非MetroCluster AFF 的各種組態。所有非MetroCluster All SAN Array（簡稱「支援」）組態、也ONTAP 支援以《支援使用支援使用支援的支援範本》（從《支援使用支援的支援範本》（從《支援使用支援的支援）為起點ASA
