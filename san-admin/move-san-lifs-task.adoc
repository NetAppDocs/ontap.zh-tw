...
summary: 如果需要將某個節點離線、您可以移動 SAN LIF 以保留其組態資訊（例如 WWPN）、並避免重新分區交換器架構。由於 SAN LIF 必須先離線才能移動、因此主機流量必須依賴主機多重路徑軟體來提供對 LUN 的不中斷存取。您可以將 SAN LIF 移至叢集中的任何節點、但不能在儲存虛擬機器之間移動 SAN LIF 。 
---
= 移動SAN LIF
:allow-uri-read: 
:icons: font
