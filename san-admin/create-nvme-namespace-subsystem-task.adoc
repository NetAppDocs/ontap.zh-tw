...
[role="lead"]
當您配置儲存設備時，您可以讓 NVMe 主機從 ONTAP 儲存系統讀取資料並將資料寫入 ONTAP 儲存系統。對於使用 NVMe 傳輸協定的系統，您必須建立一個或多個 NVMe 命名空間和子系統。然後，每個命名空間都可以對應至 NVMe 子系統，以允許從主機存取資料。

如果包含 NVMe 子系統的儲存虛擬機器（SVM）同時具有 NVMe/FC 和 NVMe/TCP 網路介面，則對應到 NVMe 子系統的命名空間可以透過 NVMe/FC 和 NVMe/TCP 傳輸協定存取。從 ONTAP 9.4 及更新版本開始，可使用 FC 的 NVMe 支援。ONTAP 9.10.1 及更新版本可使用 TCP 的 NVMe 支援。如果您需要對命名空間進行特定協定的存取，則應link:configure-svm-nvme-task.html["配置 SVM"]為每種協定單獨配置。
