...
.開始之前
您必須擁有 FC 許可證（link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["隨附於 ONTAP One"]），並且必須啓用該許可證。如果未啟用 FC 授權、則生命體與 SVM 看起來會上線、但作業狀態為 `down`。您必須啟用FC服務、才能讓您的LIF和SVM正常運作。您必須為SVM中的所有FC LIF使用單一啟動器分區、才能裝載啟動器。
