...
| SVM MetroCluster | ONTAP 9.16.1. | 您可以將 SVM 從非 MetroCluster HA 配對移轉至 MetroCluster 組態，或從 MetroCluster 組態移轉至非 MetroCluster HA 配對。您無法將 SVM 從一個 MetroCluster 組態移轉至另一個 MetroCluster 組態。[ 附註 ] SVM 移轉不支援在早於 ONTAP 9 。 16.1 的版本中進行 MetroCluster SVM 移轉。您可能可以使用 SnapMirror 非同步複寫至link:https://www.netapp.com/media/83785-tr-4966.pdf["在 MetroCluster 組態中移轉 SVM"^]。請注意，在 MetroCluster 組態中使用 SnapMirror 非同步方式來移轉 SVM 是 _ 顛覆性 _ 移轉方法。 


| NetApp Aggregate Encryption（NAE） | 不支援 | 任何使用 NAE 的端點都不支援移轉。 
