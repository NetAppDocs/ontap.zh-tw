...
Onboard Key Manager 會在叢集內部的安全資料庫中儲存及管理金鑰。其範圍為叢集。外部金鑰管理程式會在叢集外部儲存及管理金鑰。其範圍可以是叢集或儲存 VM 。

.關於這項工作
您必須使用 ONTAP 命令列介面（CLI）來設定 MetroCluster 組態的外部金鑰管理程式。深入瞭解link:configure-external-key-management-concept.html["使用 CLI 設定 ONTAP 外部金鑰管理"]。

可以使用一個或多個外部金鑰管理程式。適用下列條件：
