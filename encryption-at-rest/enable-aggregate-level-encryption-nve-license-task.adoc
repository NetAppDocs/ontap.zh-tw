...
[role="lead"]
從 ONTAP 9.7 開始、新建立的集合體和磁碟區會在您有時依預設進行加密 link:https://docs.netapp.com/us-en/ontap/encryption-at-rest/install-license-task.html["VE 授權"] 以及內建或外部金鑰管理。從ONTAP 功能區9.6開始、您可以使用Aggregate層級加密、將金鑰指派給內含的Aggregate、以便加密磁碟區。
