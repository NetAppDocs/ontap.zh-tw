...
. 修改主密鑰伺服器，新增輔助密鑰伺服器。這 `-secondary-key-servers` 此參數接受以逗號分隔的最多三個金鑰伺服器清單。輔助密鑰伺服器無需提供連接埠號碼。
...
--
. 修改主密鑰伺服器，新增輔助密鑰伺服器。這 `-secondary-key-servers` 此參數接受以逗號分隔的最多三個金鑰伺服器清單。輔助密鑰伺服器無需提供連接埠號碼。
`security key-manager external modify-server -vserver _svm_name_ -key-servers _primary_key_server_ -secondary-key-servers _list_of_key_servers_`有關輔助密鑰伺服器的更多信息，請參閱 <<mod-secondary>>。
...
輔助密鑰伺服器由以下方式管理： `-secondary-key-servers` 參數 `security key-manager external modify-server` 命令。這 `-secondary-key-servers` 參數接受以逗號分隔的清單。清單中輔助密鑰伺服器的指定順序決定了輔助密鑰伺服器的存取順序。可以透過執行以下命令來修改存取順序 `security key-manager external modify-server` 輔助密鑰伺服器的輸入順序不同。輔助密鑰伺服器無需提供連接埠號碼。
