...
summary: 建立 NFS 或 SMB 伺服器之前，請先設定 SVM 的 DNS 服務。 
---
= 設定 ONTAP 網路的 DNS 服務
:hardbreaks:
:allow-uri-read: 
...
NOTE: 為避免因 SVM FQDN 不符合 DDNS 更新的 RFC 規則而導致組態失敗，請使用符合 RFC 規則的 FQDN 名稱。如需詳細資訊，請參閱link:https://tools.ietf.org/html/rfc1123["RFC 1123"^]。
