...
summary: 所有 SSL 連線均符合 Federal Information Processing Standards 140-2。 
---
= 使用 FIPS 設定 ONTAP 網路安全性
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
|===
在啟用 FIPS 之前，必須重新配置現有的 SSH 公鑰帳戶以使用受支援的金鑰類型，否則管理員身份驗證將失敗。

如需詳細資訊、請參閱 link:../authentication/enable-ssh-public-key-accounts-task.html["啟用SSH公開金鑰帳戶"]。

ONTAP 9.18.1 引入了對 ML-KEM、ML-DSA 和 SLH-DSA 後量子計算加密演算法的支持，用於 SSL，從而為抵禦未來潛在的量子電腦攻擊提供了額外的安全保障。這些演算法僅在以下情況下可用<<停用FIPS,FIPS 已停用>>。當 FIPS 被停用且對等方支援時，將協商後量子加密演算法。
...
. 當系統提示您繼續時、請輸入 `y`
. 從 ONTAP 9.9.1 開始，無需重新啟動。如果您執行的是 ONTAP 9.8 或更早版本，請手動逐一重新啟動叢集中的每個節點。
...
您可以使用以下命令啟用 SSLv3。如果您執行的是 ONTAP 9.9.1 或更高版本，則不會看到此警告訊息。
...
.步驟
. 如果您執行的是 ONTAP 9.8 或更早版本，請手動逐一重新啟動叢集中的每個節點。
. 檢視目前的法規遵循狀態：
+
...
.相關資訊
* link:https://csrc.nist.gov/pubs/fips/203/final["FIPS 203：基於模組格的金鑰封裝機制標準（ML-KEM）"^]
* link:https://csrc.nist.gov/pubs/fips/204/final["FIPS 204：基於模組格的數位簽章標準（ML-DSA）"^]
* link:https://csrc.nist.gov/pubs/fips/205/final["FIPS 205：無狀態雜湊數位簽章標準 (SLH-DSA)"^]
* link:../networking/configure-tls-offload.html["設定 TLS 硬體卸載"]
