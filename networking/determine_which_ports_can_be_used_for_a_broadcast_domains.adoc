...
+
** 連接埠 `e0c`、 `e0d`、 `e0e`、 `e0f`和 `e0g` 在每個節點上、都會指派給預設的廣播網域。
...
+
** 節點連接埠會指派給連接埠 `e0c` 在每個節點上、叢集管理 LIF 的主節點都已開啟 `e0c` 開啟 `node1`。
** 連接埠 `e0d`、 `e0e`、 `e0f`和 `e0g` 在每個節點上都不會裝載生命空間、而且可以從預設廣播網域中移除、然後新增至新 IPspace 的新廣播網域。
