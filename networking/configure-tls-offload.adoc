...
| NVMe / TCP | 是的 | 在支援 TLS 卸載的平台和網路卡上提供支援。 


| 叢集對等加密 | 是的 | 支援叢集網路流量。這不包括 HA 流量。 


| FabricPool | 是的 | 在支援 TLS 卸載的平台和網路卡上提供支援。 


| S3 | 是的 | 在支援 TLS 卸載的平台和網路卡上提供支援。 


| 基於 TLS 的 NFS | 是的，從 ONTAP 9.19.1 開始 | 支援使用 KTLS 的 ONTAP NFS over TLS 流量。 
...
| REST API | 否 | 此設定不影響管理介面。 


| HA 流量 | 否 | TLS 硬體卸載不適用於 HA 流量。 
|===

NOTE: 在 ASA 系統上，TLS 硬體卸載僅適用於 NVMe/TCP 流量。NFS over TLS、FabricPool 和 S3 不支援於 ASA 平台。


NOTE: 在 AFX 平台上，TLS 硬體卸載支援 NFS over TLS 和 S3 前端流量類型。AFX 平台不支援 NVMe/TCP，因為 AFX 不支援 SAN/區塊協定。
