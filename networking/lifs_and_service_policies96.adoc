...
下表列出截至ONTAP 資訊版SVM中的LIF內建政策、截至資訊版9.11.1為止。對於其他版本、請使用下列命令顯示服務原則及其詳細資料：
...
預設管理
 a| 
管理核心、管理https、管理http、管理ssh、管理自動支援、 管理- EMS、管理- DNS用戶端、管理- ad用戶端、管理- LDAP用戶端、管理- NIS用戶端、 管理- NTP用戶端
 a| 
節點管理或叢集管理
 a| 
