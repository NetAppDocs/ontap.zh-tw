...
summary: LIF 容錯移轉是指當連結發生故障時，LIF 會自動移轉到不同的網路連接埠。 
---
...
使用容錯移轉群組進行 LIF 容錯移轉不支援 SAN 傳輸協定 LIF。深入瞭解link:../san-admin/multipath-automatic-lif-failover-support.html["SAN 多重路徑和自動 LIF 容錯移轉支援"]。

容錯移轉群組包含來自 ONTAP 叢集中一個或多個節點的一組網路連接埠（實體連接埠、VLAN 和介面群組）。容錯移轉群組中的網路連接埠定義了 LIF 可用的容錯移轉目標。容錯移轉群組可以指派叢集管理 LIF、節點管理 LIF、叢集間 LIF 和 NAS 資料 LIF。
...
.相關資訊
深入瞭解link:https://docs.netapp.com/us-en/asa-r2/administer/manage-client-vm-access.html#create-a-lif-network-interface["ASA r2 系統上的自動 LIF 容錯移轉"^]。
