...
summary: 從 ONTAP 9.8 開始，您可以選擇使用 IP 安全性來保護您的網路流量。IPsec 是 ONTAP 提供的多種資料傳輸加密選項之一。在生產環境中使用 IPsec 之前，您應該要做好設定準備。 
---
= 了解 ONTAP 網路上的 IP 安全性
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
如果您使用的是 ONTAP 9.16.1 或更高版本，您可以選擇將某些運算密集型作業（例如加密和完整性檢查）卸載到安裝在儲存節點上的網路介面控制器 (NIC) 卡上。與未啟用 IPsec 加密的純文字作業相比，卸載至 NIC 卡的 IPsec 加密作業對網路處理量的效能影響約為 5% 或更低。這可以顯著提升受 IPsec 保護的網路流量的效能和處理量。
...
* X50131A（IO Module，2PT，100/200GbE）
* X50131B（IO 模組，2PT，200/400GbE，B）
* X60132A （ 4p ， 10G/25G 乙太網路控制器）


ONTAP 9.17.1 增加了對以下乙太網路卡的支援：

* X50135A （IO 模組，2PT，100GbE，加密）
* X60135A（2p，40G/100G乙太網路控制器）


X50131A、X50131B 和 X50135A 卡支援以下平台：
