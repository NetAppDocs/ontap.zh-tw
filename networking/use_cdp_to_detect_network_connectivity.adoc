...
summary: 在資料中心，您可以使用鄰居發現協定來查看一對實體或虛擬系統及其網路介面之間的網路連線情況。ONTAP 支援兩種鄰居發現協定：Cisco Discovery Protocol 和 Link Layer Discovery Protocol。 
---
...
必須在所有乙太網路交換器和路由器上啟用 CDP，才能顯示有關鄰近設備的資訊。
...
NOTE: 有時，當節點上的 LIF 發生變更時，接收裝置端（例如乙太網路交換器）的 CDP 資訊不會更新。如果遇到此類問題，應將節點的網路介面設定為關閉狀態，然後再設定為開啟狀態。
