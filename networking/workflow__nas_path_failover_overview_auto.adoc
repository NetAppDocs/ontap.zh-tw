...
summary: ONTAP 9.8 及更新版本的 NAS 路徑容錯移轉組態。 
---
= ONTAP 的 NAS 路徑故障移轉工作流程
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: SAN LIF 不會遷移（除非您在連結故障後手動移動）。相反，主機上的多路徑技術會將流量轉移到不同的 LIF。如需詳細資訊，請參閱 link:../san-admin/index.html["SAN管理"]。
