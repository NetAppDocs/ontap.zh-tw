...
summary: 檢視叢集中所有作用中或正在接聽的連線、或依用戶端、LIF、傳輸協定或服務檢視節點上的作用中連線計數。 
---
= 檢視 ONTAP 叢集連線
:hardbreaks:
:allow-uri-read: 
...
深入瞭解  `network connections active show-protocols`，請參閱 link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=network+connections+active+show-protocols["指令參考資料ONTAP"^]。
