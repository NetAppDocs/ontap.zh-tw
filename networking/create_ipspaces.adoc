...
summary: 為叢集中的每個 SVM 建立 IPspace。 
---
...
[NOTE]
====
從 ONTAP 9.19.1 開始，ONTAP 透過將兩個遺失恢復功能（PRR 和 RACK）整合到 ONTAP TCP 堆疊中，提升了在高遺失率、高延遲 WAN 連結上的 TCP 資料傳輸效能。link:improve-tcp-wan-performance.html["了解如何使用 PRR 和 RACK 提升 TCP WAN 效能"]
