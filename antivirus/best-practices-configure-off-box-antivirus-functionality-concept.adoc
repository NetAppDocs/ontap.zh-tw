...
* 該 `scan-queue-timeout`值應小於 `scanner-pool request-timeout`值。如果設定過高，檔案存取可能會延遲，最終甚至逾時。為避免這種情況，請將 `scan-queue-timeout`設定為比 `scanner-pool request-timeout`值少 5 秒。有關如何變更 `scan-queue-timeout`設定的說明，請參閱掃描引擎廠商的文件。 `scanner-pool timeout`可在進階模式下使用下列命令並為 `request-timeout`參數提供適當的值來變更：
