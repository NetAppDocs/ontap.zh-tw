...
NOTE: 每個 RDMA 連接埠最多可以有 127 個 LIF。當增加更多 LIF，或當 LIF 故障轉移到超過該限制的連接埠時，新新增的 LIF 或故障轉移的 LIF 將無法處理 RDMA 流量。
...
. 選擇儲存設備虛擬機器和主節點。指定*名稱*、*IP 位址*和*子網路遮罩*。
...
** 此 `-rdma-protocols`參數接受一個列表，預設值為空。將 `roce`新增為參數值後，LIF 只能在支援 RoCE 卸載的連接埠上設定，這會影響 LIF 移轉和容錯移轉。
