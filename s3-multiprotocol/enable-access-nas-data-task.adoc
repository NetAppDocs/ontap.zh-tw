...
+
.. 按一下「Storage > Storage VMs」，選取支援 NAS 的儲存設備 VM，按一下「Settings」，然後在「Protocols > S3」下，按一下 S3 下方的 image:icon_gear.gif["動作圖示"]。
...
. 在已設定NAS傳輸協定的儲存VM上啟用S3。
+
.. 按一下 *Cluster > Storage VMs*，選擇支援 NAS 的儲存設備虛擬機器，按一下 *Settings*，然後在 *Protocols > S3* 下按一下 image:icon_gear.gif["動作圖示"]。
.. 選取憑證類型。無論您是選擇系統產生的憑證、或是自己的憑證、用戶端存取都必須使用此憑證。
.. 輸入網路介面。
...
+
** 不會再顯示秘密金鑰。
** 如果您需要再次取得憑證資訊：按一下 *Cluster > Storage VMs*，選擇儲存設備 VM，然後按一下 *Settings*。
