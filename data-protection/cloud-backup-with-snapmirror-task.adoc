...
[role="lead"]
從 ONTAP 9.9.1 開始，當您需要將資料保護工作流程遷移到雲端時，可以使用 _SnapMirror cloud_。您可以取代傳統的磁帶備份架構，使用物件儲存作為長期資料保留和歸檔的替代儲存庫。您可以選擇 Amazon S3、Alibaba Cloud、Azure Blob、Google Cloud Storage、IBM Cloud Object Storage、StorageGRID 或 ONTAP S3 等雲端物件儲存供應商。

從 ONTAP 9.9.1 開始、您可以使用 System Manager 來備份及還原資料。
