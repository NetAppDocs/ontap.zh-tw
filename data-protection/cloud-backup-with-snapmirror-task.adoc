...
[role="lead"]
從 ONTAP 9.9.1 開始，當您需要將資料保護工作流程遷移到雲端時，可以使用 _SnapMirror cloud_。您可以取代傳統的磁帶備份架構，使用物件儲存作為長期資料保留和歸檔的替代儲存庫。您可以選擇 Amazon S3、Azure Blob、Google Cloud Storage、StorageGRID 或 ONTAP S3 等供應商作為您的雲端物件儲存。
...
+
** 當您在每個叢集使用超過 6 個 bucket 時，建議對支援此功能的供應商（包括 AWS S3、Alibaba Cloud、StorageGrid、Google Cloud Storage、IBM Cloud object storage、ONTAP S3 以及其他相容 S3 的供應商）使用虛擬主機樣式的 URL 設定。建議使用此設定，以啟用可擴展的物件儲存位址並避免底層限制。
** 預設會啟用隨需還原。
