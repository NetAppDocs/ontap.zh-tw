...
* 從 ONTAP 9.19.1 開始，針對 SVM DR 所設定的 SVM 不支援啟用 IPsec。如果您正在使用 IPsec，則必須停用它或選擇其他 SVM 來設定 SVM DR 關係。如果您使用的是 ONTAP 9.19.1 或更新版本，並嘗試在設定為 SVM DR 的 SVM 上啟用 IPsec，則會收到錯誤訊息。
...
. 從來源叢集，使用 `vserver peer create`命令建立 SVM 對等關係。
+
