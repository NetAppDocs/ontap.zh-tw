...
[role="lead"]
策略類型為「vault」或「mirror-vault」的SnapMirror關係允許直接在SnapMirror關係的輔助磁碟區上建立快照。這些快照將作為備份保留在目標位置。這些快照通常是為了長期保留而創建的，稱為長期保留快照。

您可以透過在SnapMirror策略規則中指定快照建立計劃、快照名稱前綴、 SnapMirror標籤和保留計數來建立長期保留快照。無論來源上的保留規則為何，此快照都會保留在SnapMirror目標磁碟區上。

長期保留快照僅適用於FlexVol SnapMirror設定。您無法為FlexGroup SnapMirror設定建立長期保留快照。
