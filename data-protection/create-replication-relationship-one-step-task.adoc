...
[role="lead"]
從ONTAP 9.3 開始，您可以使用 `snapmirror protect`命令透過一個步驟配置資料保護關係。您指定要複製的磁碟區清單、目標叢集上的 SVM、作業計劃和SnapMirror策略。 `snapmirror protect`完成剩下的工作。
...
在ONTAP 9.4 及更高版本中，目標磁碟區最多可包含 1019 個快照。在ONTAP 9.3 及更早版本中，目標磁碟區最多可包含 251 個快照。
...
詳細了解 `snapmirror protect`在link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-protect.html["指令參考資料ONTAP"^] 。
