...
[role="lead"]
一線儲存設備中的來源磁碟區與二線儲存設備中的目的地磁碟區之間的關係稱為_DATA保護關係。_您可以使用「napmirror create」命令來建立SnapMirror DR、SnapVault ESAN或統一化複寫資料保護關係。
...
* SVM資料保護關係仍預設為DP模式。
+
明確指定XDP以使用預設的「鏡像AllSnapshots」原則來取得XDP模式。
...
. 從目的地叢集建立複寫關係：
+
"napmirror create -source-path _svm:volume（napmirror create -source-path _svm:volume）_cluster（叢集）：//SVM/volume（磁碟區）"、... -destination-path _svm:Volume _|_cluster：//svm/volume _、... 鍵入dp | xDP -schedule _schedule_-policy _`
...
----
+
以下範例SnapVault 使用預設的「XDPDefault」原則建立一個「不全」關係：
...
----
+
以下範例使用預設的「同步的（Sync）原則建立SnapMirror同步關係：
...
----
+
以下範例使用預設的「trictSync」原則建立SnapMirror同步關係：
...
----
+
以下範例建立SnapMirror DR關係。在DP類型自動轉換為XDP且未指定任何原則的情況下、原則預設為「鏡射AllSnapshots」原則：
...
----
+
以下範例建立SnapMirror DR關係。在未指定類型或原則的情況下、原則預設為「鏡像AllSnapshots」原則：
...
----
+
以下範例建立SnapMirror DR關係。若未指定原則、原則預設為「XDPDefault」原則：
...
----
+
以下範例建立SnapMirror同步關係、並使用預先定義的原則「napCenterSync」：
...
[NOTE]
====
預先定義的原則「napCenterSync」為「Sync」類型。此原則會複寫任何以「app_nistent」的「napmirror標籤」建立的Snapshot複本。
