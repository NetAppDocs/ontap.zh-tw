...
. 重新同步來源與目的地磁碟區：
+
"napmirror resSync -source-path _Svm:Volume _|_cluster：//SVM/Volume _、... -destination-path _svm:Volume _|_cluster：//svm/volume _、... 鍵入dp | xDP -schedule _schedule_-policy _`
