...
* 從ONTAP 9.13.1 開始， ONTAP預設嘗試使用快速重新同步來減少重新同步時間。若要預設啟用快速重新同步功能，必須滿足以下條件：
+
** FlexVol卷上沒有克隆卷
** 使用 MirrorAllSnapshots 策略時
+

NOTE: 使用 `-quick-resync` 由於傳輸資料區塊的儲存效率降低，重新同步目標磁碟區可能會佔用額外的空間。這部分額外的空間消耗會在重新同步目標上透過線上或複製後儲存效率應用來回收。

+
這 `-quick-resync` 參數為可選參數。您可以使用以下方法啟用或停用快速重新同步： `-quick-resync true|false` 參數 `snapmirror resync` 命令。

+
有關更多信息 `-quick-resync`參見 link:https://docs.netapp.com/us-en/ontap-cli/snapmirror-resync.html["指令參考資料ONTAP"^]。
...
====
.相關資訊
* link:resync-svm-destination-task.html["重新同步 ONTAP SnapMirror 目的地 SVM 上的資料"]
