...
| 平台 | 早於發行版本的SnapMirror 9.9.1、每個HA配對所允許的SnapMirror同步作業數ONTAP | 在SnapMirror 9.9.1中、每個HA配對允許的SnapMirror同步作業數ONTAP | SnapMirror同步作業的數量ONTAP 、可在版本號為9、10.1的每個HA配對中執行 
...
* 複寫應用程式建立的Snapshot複本
+
只有Snapshot複本具有SnapMirror標籤、符合與同步或嚴格同步原則相關的規則。使用Snapshot原則建立的排程Snapshot複本不會複寫。
...
* 混合式SAN與NAS存取
+
SnapMirror同步關係的主要磁碟區可以提供NAS資料或SAN資料。不支援從SnapMirror同步關係的主要磁碟區存取SAN和NAS。
...
* *同步模式*
+
在同步模式中、會先將I/O複寫至主要儲存設備、然後再複寫至次要儲存設備。然後將I/O寫入主儲存設備、並將確認函傳送至發出I/O的應用程式如果由於任何原因而未完成對二線儲存設備的寫入、則允許應用程式繼續寫入一線儲存設備。當錯誤情況得到修正時、SnapMirror同步技術會自動與二線儲存設備重新同步、並以同步模式從一線儲存設備恢復複寫至二線儲存設備。
...
* * StrictSync模式*
+
SnapMirror Synchronous可選擇性地在StrictSync模式中運作。如果由於任何原因未完成對二線儲存設備的寫入、應用程式I/O就會失敗、因此可確保一線與二線儲存設備完全相同。只有在SnapMirror關係恢復為「不同步」狀態之後、應用程式I/O才會繼續進行。如果主儲存設備故障、則可在容錯移轉後、在二線儲存設備上恢復應用程式I/O、而不會遺失資料。
