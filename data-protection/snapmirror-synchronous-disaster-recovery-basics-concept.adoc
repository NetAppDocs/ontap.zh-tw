...
| 自主勒索軟體保護 | ONTAP 9.19.1 | 升級後，如果對處於 SnapMirror 同步或 SnapMirror 活動同步關係中的磁碟區啟用 ARP，則該關係會短暫進入不同步狀態。這種情況是暫時的，關係會自動重新同步，無需人工幹預。如需詳細資訊，請參閱 link:../anti-ransomware/use-cases-restrictions-concept.html["自主勒索軟體保護的使用案例和考量"]。 
