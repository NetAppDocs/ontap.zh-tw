...
| 應用程式建立的快照複寫 | 更新ONTAP | 如果在 `snapshot create`操作時使用 CLI 或 ONTAP API 為快照新增了對應的標籤，則 SnapMirror 在應用程式靜默後，同步複製快照（包括使用者建立的快照和使用外部腳本建立的快照）。從 ONTAP 9.14.1 開始，複製應用程式建立的快照不再需要標籤。有關複製應用程式建立的快照的更多資訊，請參閱link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/How_to_replicate_application_created_snapshots_with_SnapMirror_Synchronous["NetApp知識庫：如何使用SnapMirror同步複製應用程式建立的快照"^]。 


| 自主勒索軟體保護 | ONTAP 9.19.1 |  
...
| NDMP / NDMP 還原 | ONTAP 9.13.1.12.9.11.9.11. | 若要對 SnapMirror 同步磁碟區執行基於傾印或 SMTape 的 NDMP 備份，來源和目的地叢集都必須執行 ONTAP 9.13.1 或更新版本。不支援對處於 SnapMirror 作用中同步或 SnapMirror 同步關係中的目的地磁碟區執行基於傾印或 SMTape 的 NDMP 還原。 
...
| Snapshot 管理作業的複寫 | ONTAP 9.19.1 | 從 ONTAP 9.19.1 開始，快照管理作業 `snapshot delete`、 `snapshot rename`和 `snapshot modify`會複寫到次要叢集。 


| 排程的 Snapshot 複寫 | ONTAP 9.19.1 | 從 ONTAP 9.19.1 開始，您可以使用 SnapMirror 原則啟用排程快照的複寫。在 ONTAP 9.19.1 之前的版本中，使用快照原則建立的排程快照不會被複寫。 
...
| SVM 災難恢復 | ONTAP 9.6  a| 
* SnapMirror 同步來源也可以是 SVM 災難恢復來源，例如，扇出組態中以 SnapMirror 同步作為一條支路，SVM 災難恢復作為另一條支路。
* SnapMirror 同步來源不能作為 SVM 災難復原目標，因為 SnapMirror 同步不支援串聯資料保護來源。在目標叢集中執行 SVM 災難復原翻轉重新同步之前，必須先解除同步關係。
* A SnapMirror 同步目標不能作為 SVM 災難復原來源，因為 SVM 災難復原不支援 DP 磁碟區的複寫。對同步來源進行翻轉重新同步會導致 SVM 災難復原將目標叢集中的 DP 磁碟區排除在外。




| link:../snaplock/snapshot-lock-concept.html["防竄改快照"] | ONTAP 9.19.1 | 您只能在建立快照時指定過期時間。您無法設定或延長現有快照的過期時間。 
...
* SnapCenter
* 資料量SnapLock
* 將基於轉儲或 SMTape 的 NDMP 還原到處於 SnapMirror 作用中同步或 SnapMirror 同步關係的目的地磁碟區
* 來源磁碟區的處理量層（QoS下限）
* Volume SnapRestore
* VVOL
...
從 ONTAP 9.19.1 開始，您可以在進階權限模式下使用 SnapMirror 策略參數 `-sync-snapshot-replication-mode`，將排程快照複寫到 SnapMirror 目的地。複寫排程快照可在 SnapMirror 同步關係中的主要與次要磁碟區之間提供對稱性。

.先決條件
* 您必須是叢集或 SVM 管理員
* 您必須處於進階權限模式
* 您必須配置一個使用者定義的 SnapMirror 原則，該原則使用參數 `-sync-snapshot-replication-mode`並包含選項 `sched_created`。
+
** 系統定義的原則無法修改。您必須建立新原則並重新建立關係。
** 使用者定義的原則變更需要重新同步（靜止後再繼續）才能對現有關係生效。
...
* `none`- 停用非 SnapMirror 同步快照的複製
* `application_created`- 複寫應用程式建立的快照
* `sched_created` - 複製排程的 Snapshot
* `sm_created` - 複製 SnapMirror 非同步（扇出分支）所建立的快照
* 以逗號分隔的類型清單： `application_created`、 `sched_created` 和  `sm_created`。


此參數僅在關係狀態為 `InSync`時生效。未列出的 Snapshot 類型僅儲存在主要磁碟區上。

從 ONTAP 9.19.1 開始， `snapmirror initialize` 會將所有快照複寫到次要叢集，而  `snapmirror resync` 只會複寫在最後一個共同快照之後建立的新快照。在重新同步作業期間，當關係狀態為  `InSync` 時標記為僅限主要的未複寫快照不會複寫到次要叢集，而這些快照會在翻轉重新同步作業後保留在新的次要叢集中。


NOTE: 在 SnapMirror 同步磁碟區上建立快照可能會增加使用者 I/O 延遲。建議您使用頻率大於 30 分鐘的快照排程。

.相關資訊
* https://www.netapp.com/pdf.html?item=/media/17174-tr4733.pdf["NetApp 技術報告 4733 ： SnapMirror 同步組態與最佳實務做法"^]
* link:https://docs.netapp.com/us-en/ontap-cli//volume-snapshot-create.html["Volume Snapshot 建立"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-delete.html["Volume Snapshot 刪除"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-modify.html["Volume Snapshot 修改"^]
* link:https://docs.netapp.com/us-en/ontap-cli/volume-snapshot-rename.html["Volume Snapshot 重新命名"^]
