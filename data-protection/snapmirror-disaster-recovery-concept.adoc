...
* 從 ONTAP 9.13.1 開始、您可以使用非同步 SnapMirror 來保護 xref:../consistency-groups/index.html[一致性群組]。從 ONTAP 9.14.1 開始、您可以使用非同步 SnapMirror 、使用一致性群組關係、將 Volume 精細快照複寫到目的地叢集。如需詳細資訊、請參閱 xref:../consistency-groups/protect-task.html#configure-asynchronous-snapmirror-protection[設定非同步 SnapMirror 保護]。
