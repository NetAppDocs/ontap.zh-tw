...
+
您可以使用命令在節點層級修改 NDMP 選項，也可以在 SVM 層級使用 `vserver services ndmp modify`命令來修改 NDMP 選項 `system services ndmp modify`。如link:https://docs.netapp.com/us-en/ontap-cli/search.html?q=services+ndmp+modify["指令參考資料ONTAP"^]需詳細 `system services ndmp modify`and `vserver services ndmp modify`資訊，請參閱。
