...
summary: 使用System Manager建立磁碟區、使用SMB傳輸協定為Windows伺服器提供儲存設備、ONTAP 此功能可搭配使用支援更新ONTAP 版本的更新版本支援。 
---
...
此程序會在上建立新的磁碟區 link:task_nas_enable_windows_smb.html["現有的SMB型儲存VM"] 並建立磁碟區根目錄（/）的共用區。您可以在設定Volume或指定自訂組態時、接受系統預設值。初始SMB組態之後、您也可以建立其他共用區並修改其內容。
...
.. 輸入名稱、選取儲存VM、然後輸入大小。
+
只會列出使用SMB傳輸協定設定的儲存VM。如果只有一個使用SMB傳輸協定設定的儲存VM可供使用、則不會顯示「*儲存VM*」欄位。
...
. [工作流程中的步驟2 ]切換至Windows用戶端、確認共用區可存取。
+
...
驗證存取之後、您可以使用共用ACL限制用戶端存取、並在對應磁碟機上設定任何所需的安全性內容。請參閱 xref:smb-config/create-share-task.html[建立SMB共用區] 以取得更多資訊。
...
+
*** xref:smb-hyper-v-sql/continuously-available-share-hyper-v-concept.html[透過SMB持續提供Hyper-V的共用需求]
*** xref:smb-hyper-v-sql/continuously-available-share-sql-concept.html[SQL Server透過SMB持續可用的共用需求]
...
若要修改現有的共用區、請按一下「**儲存區>共享區」、然後按一下 image:icon_kabob.gif["三點式veritcal kabob圖示"] 然後選取「*編輯*」。
...
+
*如果您先前在*效能服務層級*下選取*手動放置*、則無法使用此選項。否則、您要新增的磁碟區FlexVol 預設會變成一個不全的磁碟區。*磁碟區所設定之傳輸協定的存取權限。*使用SnapMirror（本機或遠端）進行資料保護、然後從下拉式清單中指定目的地叢集的保護原則和設定。*按一下「*儲存*」以建立磁碟區、並將其新增至叢集與儲存VM。
...
NOTE: 儲存磁碟區之後、請返回 <<step2>> 使用SMB完成Windows伺服器的資源配置。
...
| System Manager Classic（ONTAP 僅限版本9.7及更早版本） | link:https://docs.netapp.com/us-en/ontap-sm-classic/smb-config/index.html["SMB組態總覽"^] 


| 指令行介面ONTAP | link:smb-config/index.html["使用CLI進行SMB組態總覽"] 
|===