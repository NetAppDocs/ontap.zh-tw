...
.授權伺服器的定義方式
您可以使用任何管理介面（包括 ONTAP CLI、System Manager 或 ONTAP REST API）向 ONTAP 定義授權伺服器。例如，使用 ONTAP CLI 時，您可以使用下列命令 `security oauth2 client create`。
...
ONTAP 會驗證授權伺服器核發的 OAuth 2.0 存取權杖，並利用這些權杖對 ONTAP REST API 用戶端請求做出基於角色的存取決策。
...
您需要從 ONTAP 叢集中定義的授權伺服器取得存取權杖，才能使用 ONTAP REST API。若要取得權杖，您必須直接聯絡授權伺服器。
