...
summary: 您可以使用持續可用的 SMB 3.0 及更新版本檔案共用區，將 Hyper-V 虛擬機器檔案或 SQL Server 系統資料庫和使用者資料庫儲存在 SVM 的磁碟區上，同時為計劃性和非計劃性事件提供不中斷營運。 
---
...
* NetApp Backup and Recovery 可保護託管在 CIFS 共用上的獨立式 Hyper-V 虛擬機器、FCI 叢集執行個體，以及 System Center Virtual Machine Manager （SCVMM） 配置的虛擬機器。
+
如需使用 NetApp Backup and Recovery 保護 Hyper-V 工作負載的相關資訊，請參閱 https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-hyperv-protect-overview.html["保護 Hyper-V 工作負載概述"]。
