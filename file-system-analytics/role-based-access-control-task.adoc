...
[role="lead"]
從 ONTAP 9.12.1 開始，ONTAP 包含一個名為 `admin-no-fsa`的預先定義角色型存取控制 （RBAC） 角色。此 `admin-no-fsa`角色授予管理員層級的權限，但阻止使用者在 ONTAP CLI、ONTAP REST API 和 System Manager 中執行與 `files`端點（即檔案系統分析）相關的操作。
