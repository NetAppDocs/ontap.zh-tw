...
啟用容量分析後，ONTAP 會執行容量分析初始化掃描。此掃描會存取已啟用容量分析的磁碟區中所有檔案的元資料。掃描期間不會讀取任何檔案資料。從 ONTAP 9.14.1 開始，您可以使用 ONTAP REST API、System Manager 的 **Explorer** 索引標籤或 `volume analytics show` ONTAP CLI 命令追蹤掃描進度。若發生節流事件，ONTAP 會發出通知。
