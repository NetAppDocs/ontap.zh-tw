...
[cols="3,1,1,1,1,1,1"]
|===
|  | ONTAP 9.13.1.12.9.11.9.11. | 零點9.12.1. ONTAP | 零點9.11.1. ONTAP | 零點9.10.1 ONTAP | 部分9.9.1 ONTAP | 部分9.8 ONTAP 


| 系統管理程式中的視覺化功能 | x | x | x | x | x | x 


| 容量分析 | x | x | x | x | x | x 


| 非作用中資料資訊 | x | x | x | x | x | x 


| 支援Data ONTAP 從VMware 7-Mode轉換的Volume | x | x | x | x | x |  


| 能夠在System Manager中自訂非作用中期間 | x | x | x | x | x |  


| Volume層級活動追蹤 | x | x | x | x |  |  


| 將活動追蹤資料下載至CSV | x | x | x | x |  |  
