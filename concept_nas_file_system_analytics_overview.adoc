...
| System Manager視覺化技術| X | X | X | X | X

|容量分析| X | X | X | X | X

|非使用中資料資訊| X | X | X | X | X

|支援從Data ONTAP VMware 7-Mode | X | X | X | X | X |轉換的Volume

|可在System Manager中自訂非作用中期間| X | X | X | X |

| Volume層級活動追蹤| X | X | X ||||下載活動追蹤資料至CSV | X | X | X |||| SVM層級活動追蹤| X | X |||時間軸| X | X |||||熱門目錄 依尺寸| X ||||

|===


| =深入瞭解檔案系統分析影片：：0oRhfZIYurk[ youtube、寬=848、高=480]。進一步閱讀* link:https://www.netapp.com/media/20707-tr-4867.pdf["TR 4687：ONTAP 《關於分析檔案系統的最佳實務準則》"^]* link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Storage_Software/ONTAP_OS/High_or_fluctuating_latency_after_turning_on_NetApp_ONTAP_File_System_Analytics["知識庫：開啟NetApp ONTAP 功能的NetApp功能檔系統分析後、延遲時間會大幅或波動"^] 
|===