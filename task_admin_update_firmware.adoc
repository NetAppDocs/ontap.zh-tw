...
[role="lead"]
有了System Manager、您可以將韌體更新套用至叢集中的支援裝置、例如磁碟、磁碟櫃、磁碟限制套件（DQP）、服務處理器（SP）或基礎板管理控制器（BMC）。
...
IMPORTANT: 目前、自動韌體更新功能無法完全正常運作。您可以啟用此功能、但不會收到任何有關自動韌體更新的通知。
...
您可以設定儲存韌體更新和SP/BMC韌體更新所需執行的預設動作。
