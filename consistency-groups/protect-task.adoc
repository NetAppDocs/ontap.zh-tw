...
* 建立快照時，您可以設定一個可選的逾時值，範圍為 5 到 120 秒。如果未提供逾時值，則操作預設逾時時間為 7 秒。在 ONTAP REST API 中，使用  `action_timeout` 參數設定逾時值。在 ONTAP CLI 中，使用  `-timeout` 標誌。


.步驟
您可以使用 ONTAP REST API 完成兩階段快照，或從 ONTAP 9.14.1 開始，可以使用 ONTAP CLI。System Manager 不支援此作業。


NOTE: 如果您使用 ONTAP REST API 建立快照，則必須使用 ONTAP REST API 提交快照。如果您使用 ONTAP CLI 建立快照，則必須使用 ONTAP CLI 提交快照。不支援混合使用這兩種方法。
