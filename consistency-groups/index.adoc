...
如需 ONTAP REST API 的詳細資訊，請參閱 https://docs.netapp.com/us-en/ontap-automation/reference/api_reference.html#access-a-copy-of-the-ontap-rest-api-reference-documentation["REST API參考文件ONTAP"^]。
...
您可以在 System Manager 的一致性群組選單的**概覽**標籤中查看監控資料，也可以透過 ONTAP REST API 請求取得。從 ONTAP 9.14.1 開始，您可以使用 ONTAP CLI 的 `consistency-group metrics show`指令查看一致性群組指標。深入瞭解 `consistency-group metrics show`，請參閱link:https://docs.netapp.com/us-en/ontap-cli/vserver-consistency-group-metrics-show.html["指令參考資料ONTAP"^]。


NOTE: 在 ONTAP 9.13.1 中，您只能使用 ONTAP REST API 擷取歷史指標。從 ONTAP 9.14.1 開始，System Manager 也提供歷史指標。



== 保護一致性群組

一致性群組提供應用程式一致性保護，確保跨多個磁碟區或 LIF 的資料一致性。建立一致性群組快照時，會在該一致性群組上建立一個「圍欄」。此圍欄會啟動 I/O 佇列，直到快照作業完成，從而確保一致性群組中所有實體的資料在同一時間點的一致性。該圍欄可能會導致快照建立作業（例如排程快照原則或使用 System Manager 建立快照）期間出現短暫的延遲峰值。如需深入瞭解 ONTAP REST API 和 ONTAP CLI 的相關背景，請參閱 link:https://kb.netapp.com/on-prem/ontap/DM/REST-API["ONTAP REST API 文件"^] 和 link:https://docs.netapp.com/us-en/ontap-cli/["指令參考資料ONTAP"^]。
...
位於鏡像聚合上的一致性群組關聯的磁碟區和快照會複製到遠端網站（網站 B）。網站 B 上的磁碟區內容為一致性群組提供寫入順序保證，讓您在發生災難時可從網站 B 復原。您可以使用 ONTAP REST API 和 System Manager，在執行 ONTAP 9.11.1 或更新版本的叢集上存取一致性群組快照。從 ONTAP 9.14.1 開始，您也可以使用 ONTAP CLI 存取快照。
...
在 ONTAP REST API 中建立的一致性群組快照可以透過 System Manager 的一致性群組介面和一致性群組 ONTAP REST API 端點進行管理。從 ONTAP 9.14.1 開始，也可以使用 ONTAP CLI 管理一致性群組快照。
