...
* 從 ONTAP 9 。 14.1 開始、您可以在使用 SnapMirror 非同步時、將 Volume 精細的快照複寫到目的地叢集。使用 SnapMirror 非同步擴充一致性群組時、只有在 SnapMirror 原則為 MirrorAll 或 MirrorAndVault 時、才會在擴充一致性群組之後複寫 Volume 精細的快照。只會複寫比基準一致性群組 Snapshot 新的 Volume 精細快照。
