...
* 在 ONTAP 9.12.1 及更早版本中、您無法在 SnapMirror 主動式同步關係中將磁碟區新增至一致性群組。您必須先中斷 SnapMirror 主動同步關係、修改一致性群組、然後使用 SnapMirror 主動同步還原保護。
...
.關於這項工作
* 您無法從 SnapMirror 主動同步或 SVM 災難恢復關係中的一致性群組移除 Volume 。您必須先中斷 SnapMirror 主動同步關係、才能修改一致性群組、然後重新建立關係。
