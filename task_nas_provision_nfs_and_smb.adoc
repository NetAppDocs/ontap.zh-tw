...
+
或者、您也可以繼續啟用任何其他必要服務、例如授權、服務品質和資料保護。請參閱 <<Customize the volume configuration>>，然後返回此處完成下列步驟。

. [[step2、Step 2 in the workflow ]）]在Linux用戶端上、確認匯出功能可存取。
+
...
儲存磁碟區之後、請返回 <<step2>> 完成Windows和Linux伺服器的多重傳輸協定資源配置。
