...
summary: 從 ONTAP 9.16.1 開始，您可以為 NVMe TCP 連線啟用和停用 TLS 安全通道。 
---
...
[role="lead"]
從 ONTAP 9.16.1 開始，您可以為 NVMe/TCP 連線設定 TLS 安全的通道。您可以使用 System Manager 或 ONTAP CLI 新增啟用 TLS 的新 NVMe 子系統，或為現有 NVMe 子系統啟用 TLS。從 ONTAP 9.19.1 開始，支援 TLS 硬體卸載。
