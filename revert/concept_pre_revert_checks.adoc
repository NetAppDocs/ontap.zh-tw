...
* link:arp-warning-clear-revert.html["回應異常活動的自發勒索軟體保護警告"]。




| ONTAP 9.19.1  a| 
* 如果您有任何具有主動-主動多路徑配置的 SVM，則必須link:delete-active-active-svms.html["刪除 SVM"]。
* 如果您修改了在 ONTAP 9.19.1 或更新版本中建立的任何 QoS 原則群組的預設設定，則必須link:modify-qos-policy-groups.html["重置它們"]。
* 如果您將磁碟區升級到 ONTAP 9.19.1，並為任何使用者新增了第二個 S3 存取金鑰，則必須 link:second-s3-access-key.html["移除這些金鑰"]。




| ONTAP 9.18.1  a| 
* 如果已將 ARP/AI 的自動能力開發設定為 ONTAP 9.18.1 升級的一部分，則需要 link:anti-ransomware-disable-auto-enable.html["停用它"]。




| ONTAP 9.17.1  a| 
* 如果您已為 SAN 啟用 ONTAP ARP/AI 功能，link:anti-ransomware-disable-san.html["停用它"]。
