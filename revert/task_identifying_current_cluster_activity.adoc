...
. 找出任何無法持續使用的既有SMB工作階段： `vserver cifs session show -continuously-available Yes -instance`
+
...
----
. 如有必要、請識別您所識別的每個 SMB 工作階段所開啟的檔案： `vserver cifs session file show -session-id _session_ID_`
+
[listing]
----
