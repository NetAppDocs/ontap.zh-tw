...
summary: 若要使用集中存取原則保護對 CIFS 伺服器上資料的存取，必須採取以下幾個步驟，包括在 CIFS 伺服器上啟用動態存取控制、在 Active Directory 中設定集中存取原則、使用 GPO 將集中存取原則套用到 Active Directory 容器，以及在 CIFS 伺服器上啟用 GPO。 
---
