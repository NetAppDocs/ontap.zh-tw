...
+
ODX 複本卸載直通支援從 Hyper-V 客體作業系統起始的複本作業。客體作業系統必須支援 ODX，並使用由支援 ODX 的 SMB 或 SAN 儲存設備支援的 SCSI 虛擬磁碟。IDE 虛擬磁碟不支援 ODX 直通。來源和目的地儲存設備必須位於同一叢集中。

+
如需詳細資訊，請參閱 link:use-cases-odx-concept.html["ONTAP SMB 伺服器上 ODX 的用例"]。
