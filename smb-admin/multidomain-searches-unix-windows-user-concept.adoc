...
從 ONTAP 9.13.1 開始，當 ONTAP 在多網域名稱對應作業期間搜尋受信任網域時，它會對這些受信任網域執行站台探索，並使用探索到的站台進行網域控制器查閱。此行為有助於確保 DNS SRV 查閱針對受信任網域的網域控制器使用正確的站台資訊。如果受信任網域站台探索失敗，ONTAP 會使用 SMB 伺服器的預設站台。
