...
summary: 為了充分利用基於 Kerberos 的通訊所提供的最強安全性、您可以在 SMB 伺服器上啟用 AES-256 和 AES-128 加密。如果您不希望 SMB 伺服器在與 Active Directory KDC 進行基於 Kerberos 的通訊時選擇 AES 加密類型、您可以停用 AES 加密。預設情況下、AES 加密處於停用狀態。 
---
...
修改 `-advertised-enc-types`參數時，指定的加密類型將取代先前設定的任何類型。所有先前的加密類型設定都將被移除。

為了增強安全性，儲存設備虛擬機器 （SVM） 會在每次修改 AES 安全性選項時變更其在 AD 中的機器帳戶密碼。變更密碼可能需要包含該機器帳戶的組織單元 （OU） 的管理員 AD 認證資料。只有當 ONTAP 必須在 Active Directory 中重設機器帳戶密碼，且您的 AD 組態需要包含該機器帳戶的組織單元的管理員認證資料時，才會出現認證資料提示。如果您的 AD 已同步且已啟用自動重設，則不會出現提示。
