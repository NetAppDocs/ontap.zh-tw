...
summary: 當在 Windows 上執行的備份應用程式遇到 Unix 風格的符號連結時，會跟隨該連結並備份資料。從 ONTAP 9.15.1 開始，您可以選擇備份符號連結本身，而不是資料。此功能完全支援 ONTAP FlexGroup volumes 和 FlexVols。 
---
...
除了 ONTAP CLI 之外，您還可以使用 ONTAP REST API 管理此功能。請參閱 https://docs.netapp.com/us-en/ontap-automation/whats-new.html["ONTAP REST API 與自動化的新功能"^]以取得更多資訊。決定 ONTAP 如何處理 Unix 風格符號連結的組態，必須針對每個 SVM 分別執行。
