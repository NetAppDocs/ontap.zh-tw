...
summary: 從 ONTAP 9.3 開始、您可以修改探索網域控制器的預設程序。這可讓您將探索限制在您的站台或偏好的 DC 集區、視環境而定、可能會提升效能。 
---
...
* 只會探索本機站台中的DC。
+
可以定義 SMB 伺服器的  `default-site` 參數，以便對未在站台與服務中指派給站台的 LIF 使用此模式。從 ONTAP 9.13.1 開始，如果 ONTAP 無法探索受信任網域的站台資訊，也可以使用預設站台作為備用站台。
