...
summary: 您可以為單一共用或所有 SMB 共用設定 BranchCache。如果您選擇按共用設定，則必須在每個需要提供快取的 SMB 共用上啟用 BranchCache。 
---
...
[role="lead"]
若要使用 BranchCache，您必須設定用戶端，如果需要，還需設定分公司中用於快取內容的託管快取伺服器。您可以為單一共用或所有 SMB 共用設定 BranchCache。如果您選擇按共用設定，則必須在每個需要提供快取的 SMB 共用上啟用 BranchCache。您也可以設定 BranchCache 為在所有 SMB 共用上自動啟用快取。
...
----
.. 檢查已設定的 SVM SMB 設定，以判斷是否已啟用所有必要的 SMB 版本：
+
[source, cli]
----
