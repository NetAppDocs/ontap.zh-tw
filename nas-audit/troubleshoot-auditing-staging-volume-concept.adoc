...
summary: 當接移 Volume 或包含稽核事件日誌的 Volume 空間不足時，可能會出現問題。如果空間不足，則無法建立新的稽核記錄。啟用稽核保證後，這可能會阻止用戶端存取資料，並導致存取要求失敗。您應該了解如何疑難排解並解決這些 Volume 空間問題。 
---
...
[role="lead"]
當接移 Volume 或包含稽核事件日誌的 Volume 空間不足時，可能會出現問題。如果空間不足，則無法建立新的稽核記錄。啟用稽核保證後，這可能會阻止用戶端存取資料，並導致存取要求失敗。您應該了解如何疑難排解並解決這些 Volume 空間問題。

[NOTE]
====
啟用稽核保證後，將出現此行為。如需更多資訊，請參閱 link:../nas-audit/auditing-process-concept.html["保證稽核"]。
...
如果包含事件日誌檔案的 Volume 空間不足，稽核功能將無法將日誌記錄轉換為日誌檔案。啟用稽核保證後，這將導致用戶端存取失敗。您必須了解如何排查與事件日誌 Volume 相關的空間問題。
...
如果儲存設備虛擬機器 （SVM） 中任何包含接移檔案的 Volume 空間不足，稽核功能將無法將日誌記錄寫入接移檔案。啟用稽核保證後，這將導致用戶端存取失敗。若要解決此問題，您需要透過顯示 Volume 使用情況資訊來確定 SVM 中使用的任何接移 Volume 是否已滿。
