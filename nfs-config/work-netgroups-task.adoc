...
summary: 您可以使用 netgroup 進行使用者驗證，並在匯出原則規則中比對用戶端。您可以從外部名稱伺服器提供 netgroup 的存取權限，也可以使用 vserver services name-service netgroup load 命令將 netgroup 從統一資源識別碼載入 SVM 。 
---
