...
summary: 預設情況下，ONTAP 支援下列 NFS Kerberos 加密類型：DES、3DES、AES-128 和 AES-256。您可以使用具有 -permitted-enc-types 參數的 vserver nfs modify 命令，為每個 SVM 配置允許的加密類型，以滿足您特定環境的安全要求。 
---
