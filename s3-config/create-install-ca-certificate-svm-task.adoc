...
. 建立自我簽署的數位憑證：
+
「安全憑證create -vserver Svm_name -type root-ca -common名稱ca_cert名稱」
...
. 產生憑證簽署要求：
+
「安全性憑證genere-csr -common-name s1_server_name [addent_options]」
...
+
「安全性憑證符號-vserver Svm_name -ca_ca_cert名稱-ca-serial ca_cert _serial編號[addent_options]」
...
. 在啟用S3的SVM上安裝簽署的憑證：
+
「安全憑證安裝-類型server -vserver Svm_name」
...
. 取得公開金鑰憑證：
+
「安全證書show -vserver Svm_name -common名稱ca_cert名稱-type root-ca -instance」
