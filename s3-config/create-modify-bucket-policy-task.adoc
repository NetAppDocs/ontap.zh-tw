...
從 ONTAP 9.19.1 開始，您可以使用條件來強制執行物件上傳的條件寫入和物件刪除的條件刪除。條件寫入和刪除可以防止物件被覆寫或意外刪除。
...
----
從 ONTAP 9.19.1 開始，您可以使用條件來強制執行物件上傳的條件寫入和物件刪除的條件刪除。以下範例建立了一個儲存貯體原則陳述式條件，以強制執行 bucket1 的條件寫入：
