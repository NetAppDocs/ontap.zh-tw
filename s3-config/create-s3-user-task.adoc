...
若要讓 S3 資源的存取在原始存取金鑰過期後仍然可用，從 ONTAP 9.19.1 開始，您可以link:regenerate-access-keys.html#generate-a-second-s3-access-key["產生第二個存取金鑰"]為每個 S3 使用者執行此操作。
...
----
+
建立使用者時，您也可以手動在 `-access-key`和 `-secret-key`參數中定義自己的存取金鑰和私鑰。例如：
...
.系統管理員
--
. 選擇 *Storage > Storage VMs*。選擇需要新增使用者的 Storage VM，選擇 *Settings*，然後在 S3 下選擇 image:icon_pencil.gif["編輯圖示"]。
. 若要新增使用者，請選取 *Users > Add*。
. 輸入使用者名稱。
. 從 ONTAP 9.14.1 開始、您可以指定為使用者建立之存取金鑰的保留期間。您可以指定金鑰自動過期的保留期間（以天、小時、分鐘或秒為單位）。依預設、此值設為 `0` 這表示金鑰無限期有效。
. 選擇 *Save* 。使用者建立完成，系統將為該使用者產生存取金鑰和私鑰。
