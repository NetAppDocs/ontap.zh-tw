...
*** 如果您未為新儲存桶指定 Aggregate 和組成元素，則新儲存桶將新增至現有的 FlexGroup，但前提是 `retention-mode`設定與現有儲存桶的 `retention-mode`設定和 FlexGroup 類型 (`compliance`、 `non-snaplock`、 `enterprise`）相符。如需詳細資訊，請參閱 xref:../flexgroup/index.html[資料區管理FlexGroup]。
+
