...
如果在 Bucket 原則陳述中指定的 nasgroup 或 nasgroup 的使用者沒有 UID 和 GID 集、當找不到這些屬性時、查詢將會失敗。Active Directory 使用 SID ，而非 UID 。如果無法將 SID 項目對應至 UID ，則必須將必要資料帶到 ONTAP 。

若要這麼做，請使用 link:../authentication/enable-ad-users-groups-access-cluster-svm-task.html["vserver Active Directory建立"]SVM 來驗證 Active Directory ，並取得必要的使用者和群組資訊。
...
NOTE: 將 API 導向叢集管理 LIF ，而非 SVM 的資料 LIF 。
