...
* 嵌套 QoS 策略
+
link:../performance-admin/policy-group-assignment-restrictions.html["QoS 策略群組分配限制"]
...
其他功能
 a| 
* 支援對組成 FlexGroup Volume 的各個成員 Volume 進行 Volume 移動。
...
* FabricPool 雲端層（link:../s3-config/index.html["僅限原生 S3"]）
* FabricPool 本機層（link:../s3-multiprotocol/index.html["僅限 NAS 磁碟區"]）
* link:../flexcache/supported-unsupported-features-concept.html#supported-features["資料量FlexCache"]（從 ONTAP 9.18.1 開始）

 a| 
* S3 儲存桶所使用的 FlexGroup Volume 不支援 Volume 移動。
* FPolicy
* qtree
* 配額
