...
[role="lead"]
從功能不只9.8開始ONTAP 、您可以在ONTAP 一個叢集中啟用一個功能不易用的儲存服務（S3）物件儲存伺服器ONTAP 。如需詳細資訊、請參閱 xref:s3-support-concept.html[S3支援ONTAP 支援支援功能。9.]。
...
如需FabricPool 有關資料分層的詳細資訊、請參閱 xref:../concept_cloud_overview.html[系統管理程式的架構管理總覽FabricPool]。
