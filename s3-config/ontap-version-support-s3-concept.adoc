...
與其他協定（例如 FC、iSCSI、NFS、NVMe_oF 和 SMB）一樣，S3 也需要先安裝授權才能在 ONTAP 中使用。S3 授權是免費授權，但必須安裝在升級至 ONTAP 9.8 的系統上。您可以從 link:https://mysupport.netapp.com/site/systems/master-license-keys/ontaps3["授權金鑰頁面"^] 在 NetApp 支援網站下載 S3 授權。
