...
| DeleteBucketCors | 部分9.8 ONTAP 


| 刪除 BucketLifecycle | 部分9.8 ONTAP 
...
| PuttBucketLifecycleConfiguration | ONTAP 9.13.1 * 僅支援過期操作 
...
從 ONTAP 9.19.1 開始，ONTAP S3 支援對物件上傳強制執行條件式寫入，以及對物件刪除強制執行條件式刪除。條件式寫入和刪除提供了一種防止物件被覆寫或意外刪除的方法。條件式作業僅在透過 TLS 連線使用 AWS_V4 簽署方法提出要求時才有效。支援下列條件式標頭和物件作業：
...
| DeleteObjects ONTAP S3 支援此附加標頭：* `x-amz-bypass-governance-retention` | 零點9.11.1. ONTAP 
...
* ONTAP S3 新增支援使用  `x-amz-content-sha256:
STREAMING-AWS4-HMAC-SHA256-PAYLOAD` 的分塊上傳簽章要求
