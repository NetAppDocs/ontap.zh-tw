...
. 將用戶端組態與虛擬伺服器建立關聯。請參閱 link:../nfs-config/enable-ldap-svms-task.html["將LDAP用戶端組態與SVM建立關聯"] 和 https://docs.netapp.com/us-en/ontap-cli//vserver-services-name-service-ldap-create.html["Vserver服務名稱服務LDAP建立"]。
. 如果您使用的是資料儲存 VM 、請在 VM 上建立管理網路介面（ LIF ）、以及 LIF 的服務原則。請參閱 https://docs.netapp.com/us-en/ontap-cli/network-interface-create.html["建立網路介面"] 和 https://docs.netapp.com/us-en/ontap-cli//network-interface-service-policy-create.html["建立網路介面服務原則"] 命令。
...
----
+
如需此命令的詳細資訊、請參閱 link:https://docs.netapp.com/us-en/ontap-cli/security-login-rest-role-create.html["建立安全登入REST角色"] 命令。

. 使用安全登入命令建立 LDAP 使用者群組、並新增新的自訂角色以存取 S3 使用者 REST API 端點。如需此命令的詳細資訊、請參閱 link:https://docs.netapp.com/us-en/ontap-cli//security-login-create.html["建立安全登入"] 命令。
+
[listing]
----
