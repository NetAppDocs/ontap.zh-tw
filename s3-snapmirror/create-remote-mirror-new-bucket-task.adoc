...
summary: 建立新的 S3 儲存區時、您可以立即將其保護到遠端叢集上的 SnapMirror S3 目的地。 
---
= 建立新儲存區（遠端叢集）的鏡射關係
:hardbreaks:
:toclevels: 1
...
[role="lead"]
建立新的 S3 儲存區時、您可以立即將其保護到遠端叢集上的 SnapMirror S3 目的地。
...
. 在來源叢集上，如果您沒有現有的 SnapMirror S3 原則，而且不想使用預設原則，請建立該原則：
+
...
----
. 在來源 SVM 上、如果您沒有現有的 SnapMirror S3 原則、而且不想使用預設原則、請建立該原則：
