...
NOTE: 您可以建立一個FlexCache磁碟區作為啟用了FabricPool分層功能的來源磁碟區的緩存，但FlexCache磁碟區本身不能分層。
...
從ONTAP 9.5 開始支援。SVM DR 關係的主 SVM 可以具有來源磁碟區；但是，如果您執行的是ONTAP ONTAP之前的版本，當 SVM DR 關係中斷時，必須使用新的來源磁碟區重新建立FlexCache關係。

從ONTAP 9.18.1 開始，當來源 SVM 發生故障轉移時，DR 站點的快取會自動切換到來源。無需手動恢復步驟。

xref:create-volume-task.adoc[了解如何建立FlexCache磁碟區。]
 a| 
否

您可以在主要 SVM 中擁有 FlexCache 來源磁碟區，但不能在次要 SVM 中擁有。FlexCache 關係不會被複寫，但來源磁碟區資料會以一般磁碟區的形式進行複寫。
...
NOTE: 在 ONTAP 9 9.5 之前的版本中，來源 FlexVol 磁碟區只能為在以 7-Mode 運作的 Data ONTAP 8.2.x 系統上建立的 FlexCache 磁碟區提供資料服務。從 ONTAP 9.5 開始，來源 FlexVol 磁碟區也可以為 ONTAP 9 系統上的 FlexCache 磁碟區提供資料服務。如需從 7-Mode FlexCache 移轉至 ONTAP 9 FlexCache 的相關資訊，請參閱 link:https://www.netapp.com/pdf.html?item=/media/7336-tr4743.pdf["NetApp 技術報告 4743 ： FlexCache in ONTAP"^]。
