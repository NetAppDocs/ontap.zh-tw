...
* SnapMirror關係中的主要Volume或次要Volume不能FlexCache 是一個SnapMirror Volume。
* SnapMirror關係的主要磁碟區只能是FlexCache 一個來源不全的磁碟區。
+
從功能不全的9.8開始ONTAP 、SnapMirror次要Volume可以是FlexCache 一個來源不全的Volume。

* 您可以在FlexCache SnapMirror關係中、從來源主要Volume取得一個不屬於次要Volume的SnapMirror Volume。
