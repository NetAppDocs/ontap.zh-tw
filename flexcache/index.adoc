...
從 ONTAP 9.19.1 開始，當數百到數千個用戶端同時列出同一個小型 NAS 目錄（2MB 或更小，約 25,000 個檔案）時，效能將會提升。此增強功能減少了使用本機 FlexCache 磁碟區等變通方案來支援大量用戶端執行目錄清單操作的需求。對於較少的用戶端數量，效能不會受到負面影響。ONTAP 9.19.1 允許使用更多 CPU 資源來處理小型 NAS 目錄的清單請求。此效能增強功能是自動的，無需任何配置。
