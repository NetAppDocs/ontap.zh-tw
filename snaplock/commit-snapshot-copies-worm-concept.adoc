...
.步驟
. 識別目的地叢集。
. 在目的地叢集上、如SnapLock 所述、安裝不含資訊的授權、初始化「完成時鐘」、並建立SnapLock 一個不含資訊的Aggregate xref:configure-snaplock-concept.adoc[設定SnapLock 功能]。
. 在目的地叢集上、建立SnapLock 一個尺寸等於或大於來源Volume的「DP」類型的「Sf目的地Volume」：
+
‘* Volume create -vserver _Svm_name_-volume _volume _name_-Aggregate _Aggregate name_-type DP -size _size_*

+
[NOTE]
====
從Aggregate繼承而來的是「法規遵循」或「企業」等功能。SnapLock不支援具有版本彈性的目的地Volume。目的地Volume的語言設定必須符合來源Volume的語言設定。
