...
summary: '您可以使用 volume `-snaplock-type` 選項來指定 Compliance 或 Enterprise SnapLock Volume 類型。對於早於 ONTAP 9.10.1 的版本、您必須建立個別的 SnapLock Aggregate 。從 ONTAP 9.10.1 開始、SnapLock 和非 SnapLock Volume 可以存在於同一個 Aggregate 上。因此、如果您使用的是 ONTAP 9.10.1 、則不再需要建立個別的 SnapLock Aggregate 。' 
---
= 建立ONTAP SnapLock聚合
:allow-uri-read: 
:icons: font
