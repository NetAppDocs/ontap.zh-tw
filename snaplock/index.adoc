...
使用 SnapLock，您可以將檔案和快照提交至 WORM 儲存設備，並設定 WORM 保護資料的保留期限。SnapLock WORM 儲存採用 NetApp 快照技術，並可利用 SnapMirror 複寫和 SnapVault 備份作為基礎技術，為資料提供備份復原保護。深入瞭解 WORM 儲存設備：link:https://www.netapp.com/pdf.html?item=/media/6158-tr4526.pdf["符合WORM儲存設備、使用NetApp SnapLock 技術- TR-4526"^]。
...
SnapLock是ONTAP的一項基於許可證的功能。一個許可證即可讓您在嚴格的合規模式下使用SnapLock ，以滿足 SEC 規則 17a-4(f) 等外部要求，以及在較為寬鬆的企業模式下使用 SnapLock，以滿足內部強制規定的數位資產保護要求。SnapLock許可證是以下部分的一部分： link:../system-admin/manage-licenses-concept.html#licenses-included-with-ontap-one["ONTAP One"] 軟體套件。
