...
.步驟
. 識別目的地叢集。
. 在目的地叢集上、安裝SnapLock 該資訊技術授權、初始化「完成時鐘」、然後建立SnapLock 一個「不中斷彙總」。
...
[NOTE]
====
這個功能是從Aggregate繼承而來的、亦即「法規遵循」或「企業」SnapLock 。不支援具有版本彈性的目的地Volume。目的地Volume的語言設定必須符合來源Volume的語言設定。
