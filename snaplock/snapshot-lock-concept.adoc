...
--
. 瀏覽至*儲存>磁碟區*。
. 選取 image:icon_kabob.gif["功能表選項"] 然後選擇 * 編輯 > Volume * 。
...
. 選取 * 設定 * 。
. 找到 * Snapshot policies * 、然後選取 image:icon_arrow.gif["箭頭"]。
. 在 * 新增 Snapshot Policy* 視窗中、輸入原則名稱。
. 選取 image:icon_add.gif["新增"]。
...
. 瀏覽至 * 儲存 > 磁碟區 * 、然後選取磁碟區。
. 在 Volume 詳細資料頁面中、選取 * Snapshot Copies （快照複本） * 標籤。
. 選取 Snapshot 複本、然後選取 image:icon_kabob.gif["功能表選項"]，然後選擇 * 修改 SnapLock 到期時間 * 。您可以選取行事曆來選擇保留到期日和時間。
. 儲存您的變更。
. 在「 * 磁碟區 > Snapshot Copies （快照複本）」頁面中、選取 * 顯示 / 隱藏 * 、然後選擇 * SnapLock Expiration Time （過期時間） * 以顯示 * SnapLock Expiration Time* （ * 快照過期時間）欄、並確認已設定保留時間。
