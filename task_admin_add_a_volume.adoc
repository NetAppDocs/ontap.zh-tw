...
---
= 使用 ONTAP System Manager 新增 Volume
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
+
** *透過 NFS 匯出*：建立具有 `default`匯出原則的 Volume，該原則授予使用者對資料的完全存取。
...
NOTE: 您也可以將此 Volume 的規格儲存到 Ansible Playbook 中。如需更多詳細資料，請前往 link:task_use_ansible_playbooks_add_edit_volumes_luns.html["使用「可執行教戰手冊」來新增或編輯磁碟區或LUN"]。
