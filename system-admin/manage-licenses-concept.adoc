...
summary: 授權是一項或多項軟體權利的記錄。從 ONTAP 9.10.1 開始，所有授權均以 NetApp 授權檔案的形式交付，這是一個可啟用多種功能的單一檔案。自 2023 年 5 月起，所有 AFF 系統和 FAS 系統均隨附 ONTAP One 軟體套件或 ONTAP Base 軟體套件進行銷售；自 2023 年 6 月起，所有 ASA 系統均隨附 ONTAP One for SAN 進行銷售。 
---
= ONTAP 授權總覽
:allow-uri-read: 
:icons: font
...
ONTAP One 包含所有可用的授權功能。它整合了先前的 Core 套裝組合、Data Protection 套裝組合、Security and Compliance 套裝組合、Hybrid Cloud 套裝組合和 Encryption 套裝組合的內容。加密功能在受限國家/地區不可用。
...
從 2023 年 6 月開始，使用 28 個字元授權金鑰的 ONTAP 系統也可以link:https://kb.netapp.com/onprem/ontap/os/How_to_get_an_ONTAP_One_license_when_the_system_has_28_character_keys["升級至 ONTAP One 或 ONTAP Base 相容性套件"^]。
