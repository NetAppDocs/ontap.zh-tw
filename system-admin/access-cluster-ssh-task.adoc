...
+
 `-application`命令參數[`security login`會指定使用者帳戶的存取方法。深入瞭解 ONTAP 命令參照中的連結： https://docs 。 NetApp 。 ONTAP 。 CLI/security-login-create.html#description[`security login`^] 命令。
