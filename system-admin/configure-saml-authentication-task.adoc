...
summary: 從 ONTAP 9.3 開始、您可以為 Web 服務設定安全性聲明標記語言驗證。 
---
...
NOTE: 某些 URL 包含問號 (?) 字元。問號會啟動 ONTAP 命令列主動說明。若要輸入包含問號的 URL、您必須先使用命令停用主動說明 `set -active-help false`。稍後可以使用命令重新啟用主動說明 `set -active-help true`。如需深入瞭解、請參閱link:https://docs.netapp.com/us-en/ontap-cli/set.html["指令參考資料ONTAP"^]。
