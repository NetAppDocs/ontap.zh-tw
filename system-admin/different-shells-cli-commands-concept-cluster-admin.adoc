...
可以使用以下方式存取叢集 shell 中支援的 Nodeshell 選項： `vserver options clustershell` 命令。若要查看這些選項，您可以使用下列命令查詢 clustershell CLI： `vserver options -vserver _nodename_or_clustername_ -option-name ?`
