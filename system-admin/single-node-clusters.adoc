...
summary: 單節點叢集是在獨立節點上執行的叢集特殊實作。不建議使用單節點叢集，因為它們不提供備援。如果節點故障，資料存取就會中斷。 
---
= 管理單節點 ONTAP 叢集
:allow-uri-read: 
:icons: font
...
.相關資訊
https://www.netapp.com/pdf.html?item=/media/10681-tr4684.pdf["NetApp 技術報告 4684 ：使用 NVMe 實作及設定現代化 SAN"^]
