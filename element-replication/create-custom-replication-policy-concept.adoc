...
+
從 ONTAP 9 開始、您可以使用 `-common-snapshot-schedule`參數來指定建立 SnapMirror 同步關係通用 Snapshot 複本排程的排程。根據預設、 SnapMirror 同步關係的一般 Snapshot 複本排程為一小時。您可以為 SnapMirror 同步關係的 Snapshot 複本排程指定 30 分鐘到兩小時的值。
