...
---
= 使用適用於 SAN 的 ONTAP SnapMirror 主動同步進行保護
:hardbreaks:
:toclevels: 1
:allow-uri-read: 
:toclevels: 1
...
使用 SnapMirror 主動同步為 SAN 組態設定非對稱保護，涉及在 ONTAP 來源叢集上選取 LUN 並將其新增至一致性群組。
...
* 主要和次要 SVM 必須處於對等關係中。
* 來源和目的地叢集不能是 MetroCluster 組態的一部分。
...
您可以使用 System Manager 或 ONTAP CLI 為 SAN 組態建立對稱保護。這兩個介面都有不同的步驟xref:index.html#key-concepts[統一且不一致的組態]。
...
--
.統一組態的步驟
. 在主站點上，link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["使用新的 LUN 建立一致性群組"]。
+
...
.系統管理員
--
. 在主站點上，link:../consistency-groups/configure-task.html#create-a-consistency-group-with-new-luns-or-volumes["使用新的磁碟區或 NVMe 命名空間建立一致性群組"]。
