...
summary: 如果要透過在 SAN 組態中新增或刪除 Volume 來變更一致性群組的組成，則必須先刪除原始關係，然後使用新的組成重新建立一致性群組。 
---
= 新增或刪除ONTAP一致性群組或刪除卷
:hardbreaks:
:allow-uri-read: 
...
[role="lead"]
隨著應用程式工作負載需求的變化，您可能需要在一致性群組中新增或移除 Volume，以確保 SAN 組態的業務連續性。在雙主動式 SnapMirror 主動同步關係中新增和移除 Volume 的程序，取決於您所使用的 ONTAP 版本。
