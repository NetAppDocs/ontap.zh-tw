...
A 系列， C 系列
 a| 
自動故障轉移雙工 （對稱主動 / 主動）、自動故障轉移 （非對稱主動 / 被動）
 a| 
* 2（ONTAP 9.9.1 或更高版本）
...
* 在 ONTAP 9.15.1 之前的版本中，讀寫目標磁碟區（從非對稱主備模式下的 DP 磁碟區轉換為讀寫磁碟區）不支援 SnapMirror 活動同步關係。要使用讀寫磁碟區，必須先將其轉換為 DP 磁碟區，方法是建立磁碟區層級 SnapMirror 關係（非同步或同步），然後刪除該關係。有關詳細資訊，請參閱 link:convert-active-sync-task.html["將現有的 SnapMirror 關係轉換成 SnapMirror 主動同步"]。
