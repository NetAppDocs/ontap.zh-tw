...
---
= 將現有的 ONTAP SnapMirror SAN 關係轉換為 SAN 關係的 SnapMirror 主動同步
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
如果您已設定 SnapMirror 保護，則可以將 SAN 關係轉換為 SnapMirror active sync for SAN。從 ONTAP 9.15.1 開始，您可以將該關係轉換為使用對稱主動/主動保護。

有關將 SVM-DR 關係轉換為 NAS 的 SnapMirror 主動同步關係的資訊，請參閱 link:protect-nas-task.html#convert-an-svm-dr-relationship-to-a-snapmirror-active-sync-relationship["將 SVM DR 關係轉換為 SnapMirror 主動同步關係"]。
...
.開始之前
* 主集群和輔助集群之間必須存在零 RPO SnapMirror同步關係。
* 必須先取消對應目的地磁碟區上的所有 LUN 、才能建立零 RTO SnapMirror 關係。
* link:mediator-install-task.html["資訊媒體ONTAP"] 必須為SnapMirror主動同步進行設定。
