...
* ONTAP Mediator 1.12.1 或更新版本安裝在具有 2 核心 CPU 的主機上。
* ONTAP Mediator 執行個體已設定 HTTPS 協定。
+
...
IMPORTANT: 如果您正在使用 SnapMirror active sync 和 ONTAP Mediator 或 ONTAP Cloud Mediator 與 ONTAP 9.17.1，則應查看 link:https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["發行說明 ONTAP"^] 中的 *已知問題和限制*，以了解有關這些組態的重要資訊。
...
使用 ONTAP Mediator 監控 SnapMirror 主動同步關係時，如果沒有有效的自簽名憑證或憑證授權單位（CA）憑證，則無法在 ONTAP 中初始化 ONTAP Mediator。您需要將有效憑證新增至對等叢集的憑證儲存區。
...
----
. 重複上一步，在對等叢集上安裝根 CA 憑證和中間 CA 憑證。
