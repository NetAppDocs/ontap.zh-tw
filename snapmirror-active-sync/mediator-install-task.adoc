...
使用 ONTAP Mediator 監控 SnapMirror 主動同步關係時，如果沒有有效的自簽名憑證或憑證授權單位（CA）憑證，則無法在 ONTAP 中初始化 ONTAP Mediator。您需要將有效憑證新增至對等叢集的憑證儲存區。使用 ONTAP Mediator 監控 MetroCluster IP 系統時，初始設定後不再使用 HTTPS；因此，無需憑證。
