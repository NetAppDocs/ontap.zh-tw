...
* 在刪除 SnapMirror 主動同步 SAN 關係之前，必須先取消對應目的地叢集中的所有 LUN。
...
. 在「刪除複寫組態」視窗中，勾選「釋放來源 Volume 基本快照」，然後選取「刪除」。
...
. 在來源叢集上，導覽至*保護>複製*。
. 找到要刪除的關係，選擇image:icon_kabob.gif["功能表選項圖示"]並選擇*刪除*。
. 在「刪除複寫組態」視窗中，勾選「釋放來源儲存設備 VM 基本快照」，然後選取「刪除」。
