...
從 ONTAP 9.15.1 開始，SnapMirror 主動同步支援在容錯移轉事件發生後自動重新組態連出連結。如果從主要站台到次要站台的容錯移轉成功，第三站台將自動重新組態，以將次要站台視為來源。非同步連出連結可以是 SAN 組態中的一致性群組關係，也可以是獨立的 Volume 關係。重新組態適用於這兩種情況。重新組態由計劃內或計劃外容錯移轉觸發。容錯回復至主要站台時也會進行重新組態。
