...
如需 NAS 組態的其他支援資訊，請參閱link:protect-nas-task.html["使用 ONTAP SnapMirror active sync for NAS 進行保護"]。
...
* AIX
+
** 從 ONTAP 9.19.1 版本開始，對稱主動 / 主動組態支援透明應用程式容錯移轉（TAF）。不需要 PVR。
** 從 ONTAP 9.11.1 到 ONTAP 9.18.1，AIX 僅由 PVR 支援對稱主動/主動組態的自動容錯移轉。
...
從 ONTAP 9.19.1 開始，AIX 的透明應用程式容錯移轉在對稱主動/主動組態中透過 SnapMirror 主動同步獲得支援。您可以使用 System Manager 或 ONTAP CLI 建立 AIX LUN。為確保透明容錯移轉，當您使用 ONTAP CLI 時，必須在 lun create 或 lun modify 命令中使用選項 access-mode local。預設情況下，存取模式設定為 global。當您使用 System Manager 時，請選擇 AIX 作為主機，並選擇 local 作為存取模式。如需建立 LUN 的詳細資訊，請參閱link:../san-admin/provision-storage.html["配置SAN儲存設備"]。如需 AIX 支援的其他資訊，請參閱 NetApp 知識庫文章：link:https://kb.netapp.com/on-prem/ontap/DP/SnapMirror/SnapMirror-KBs/SnapMirror_active_sync_support_for_AIX%3A_behavior_by_ONTAP_release_and_migration_guidance["SnapMirror 主動同步對 AIX 的支援：依 ONTAP 版本的行為與遷移指南"^]。

如果您使用的是 ONTAP 9.11.1 至 ONTAP 9.18.1，則可透過標準工程功能策略變更請求（FPVR）支援 AIX 的 SnapMirror 主動式同步，但前提是瞭解以下規定：
...
當主叢集與次要叢集之間的連線中斷，且主叢集與調解器之間的連線也中斷時，隔離的主叢集上可能會因雙重事件故障而發生自動非計畫性容錯移轉 (AUFO) 事件。與其他 AUFO 事件不同，這種情況較為罕見。
...
為確保在SnapMirror主動同步環境中發生計劃外站點故障轉移切換時 Solaris 用戶端應用程式不會中斷，請修改預設 Solaris 作業系統設定。若要使用建議的設定配置 Solaris，請參閱link:https://kb.netapp.com/Advice_and_Troubleshooting/Data_Protection_and_Security/SnapMirror/Solaris_Host_support_recommended_settings_in_SnapMirror_Business_Continuity_(SM-BC)_configuration["NetApp知識庫：Solaris 主機支援SnapMirror主動同步中的建議設置"^]。
...
從 ONTAP 9.19.1 開始，ARP/AI 支援參與 SnapMirror 主動同步 SAN 關係的主要 Volume。如需詳細資訊，請參閱 link:../anti-ransomware/use-cases-restrictions-concept.html#interoperability-and-vm-considerations["自主勒索軟體保護的使用案例和考量"]。
...
SnapMirror SAN 的主動同步支援xref:../data-protection/supported-deployment-config-concept.html[連出組態]搭配 `MirrorAllSnapshots`原則，從 ONTAP 9.11.1 開始，也支援 `MirrorAndVault`原則。連出組態不支援搭配 `XDPDefault`原則的 SnapMirror 主動同步。

從 ONTAP 9.15.1 開始，SnapMirror 主動同步支援在故障切換事件發生後自動重新設定連出連結。如果從主要站台到次要站台的故障切換成功，則在非同步連出連結的下一次排程更新期間，將啟動連出關係的重新設定。它不會在 SnapMirror 主動同步故障切換後立即觸發。非同步連出連結可以是 SAN 組態中的一致性群組關係，也可以是獨立的 Volume 關係。重新設定適用於這兩種情況。重新設定由計劃內或計劃外故障切換觸發。容錯回復至主要站台時也會進行重新設定。
...
.部分檔案 SnapRestore
從 ONTAP 9.12.1 開始，link:../data-protection/restore-part-file-snapshot-task.html["部分 LUN 還原"]支援 SnapMirror 主動同步 Volume。您可以從應用程式建立的快照中還原資料，這些快照已在 SnapMirror 主動同步來源（Volume）和目的地（快照）Volume 之間複寫。如果您需要在主機上還原資料庫，而該主機在同一個 LUN 上儲存多個資料庫，則可能需要進行部分 LUN 或檔案還原。使用此功能需要您知道資料的起始位元組偏移量和位元組計數。
...
SnapMirror 主動同步不支援防篡改 Snapshot 。
