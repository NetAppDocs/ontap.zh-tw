...
從 ONTAP 9.19.1 開始，AIX 的透明應用程式容錯移轉（Transparent Application Failover）在對稱主動/主動組態中支援 SnapMirror 主動同步。您可以使用 System Manager 或 ONTAP CLI 建立 AIX LUN。為確保透明容錯移轉,在使用 ONTAP CLI 時,必須在 lun create 或 lun modify 命令中使用 access-mode local 選項。預設情況下,存取模式設定為 global。使用 System Manager 時,請選擇 AIX 作為主機,並將存取模式設定為 local。有關建立 LUN 的更多資訊,請參閱link:../san-admin/provision-storage.html["配置SAN儲存設備"]。
