...
在非對稱主動 / 被動配置中，次要站點上的資料被代理到 LUN、命名空間或儲存單元。
...
自 ONTAP 9.15.1 起， SnapMirror active sync 支援主要優先（Primary-first）與主要偏好（primary bias）。主要複本是在 System Manager 中指定，並透過 ONTAP REST API 和 ONTAP CLI 輸出。
...
NOTE: 非對稱主動 / 被動模式不支援 ASA r2 和 NVMe。有關 NVMe 支援的更多資訊，請參閱 link:../nvme/support-limitations.html["NVMe 組態、支援和限制"]。
...
* FC
 ...
 a| 
* 非對稱主動 / 被動
* 對稱主動 / 主動配置在 ONTAP 9.16.1 及更高版本中支援 4 節點到 4 節點的叢集。對於 ASA r2 ，僅支援 2 節點叢集。
...
* FC
 ...
 a| 
* 非對稱主動 / 被動
* 對稱主動 / 主動對稱主動 / 主動配置支援 ONTAP 9.15.1 中的 2 節點叢集。ONTAP 9.16.1 及更新版本支援 2 節點到 2 節點叢集和 4 節點到 4 節點叢集。
