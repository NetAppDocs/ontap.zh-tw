...
. 對於 SAN 組態，在兩個叢集的每個節點上至少建立一個 SAN LIF（根據情況選擇 FC 或 iSCSI），或在主要叢集的每個節點上建立至少一個 NAS LIF（根據情況選擇 NFS 或 SMB），以便複寫到次要叢集。
+
...
+
link:../nfs-config/index.html["了解 NFS 組態"]

+
link:../smb-config/index.html["了解 SMB 組態"]

. 僅限 SAN：建立必要的 LUN 並將其對應到 igroup，該 igroup 用於將 LUN 對應到應用程式主機上的啟動器。
+
link:../san-admin/provision-storage.html["建立LUN並對應igroup"]

. 重新掃描應用程式主機，以發現任何新的 LUN 或 Volume。
